...
**v0.8.0** — established 2026-05-16 (Minor). Bulk actions on the Clients tab, built as an **action-first** flow: the Clients list looks unchanged until the advisor picks a bulk operation from the ☰ menu, and only then does a selection UI appear. This is Chat 4 of the parallel-chat workplan (WORKPLAN §3).

1. **Action-first selection — no persistent selection UI.** By default the Clients tab renders exactly as before: a plain client list, no checkboxes, no selection bar. Row click opens `ClientDetail`. The bulk feature only activates when the advisor opens the Clients-tab ☰ menu and picks an action. `ClientList` was rewritten from a single dense line into a multi-line component to hold the new flow logic. (An earlier draft of this release used an always-visible "select first, then act" selection bar; Mauricio asked for action-first instead, so the persistent selection UI was removed before release — v0.8.0 ships only the action-first version.)

2. **Five operations in the Clients-tab ☰ menu.** The existing `ClientList` Kebab (Import/Export/Backup/Restore utilities) gains a divider and five items: 📦 Archive…, ↩ Restore…, 🗑️ Delete…, ✂️ Split…, 🔗 Join…. **All five are always enabled** — there is nothing to gate them on, because selection happens *after* the menu pick, not before. (The shared `Kebab` component is therefore untouched this release — no `disabled`-item support was needed.)

3. **Selection mode (Archive / Restore / Delete).** Picking Archive, Restore, or Delete puts `ClientList` into a selection `mode`. A coloured action bar appears above the list ("Select clients to archive", a select-all checkbox, a live count, Cancel, and the action button), and checkboxes appear on the rows the action can apply to. **Selectability is scoped to the action:** Archive shows checkboxes on active clients only; Restore shows them on archived clients only (and auto-expands the Archived section); Delete shows them on every client. Rows that the current action cannot touch are dimmed and inert. Selection is component-local state (`useState(()=>new Set())` of client ids) — not persisted, not lifted to App, cleared on Cancel or completion. The action button opens a confirmation `Modal` listing every affected client by name; Delete additionally requires typing `DELETE` exactly before its destructive button enables (same guard as the per-client `DeleteClientModal`). Cancel exits the mode and restores the plain list.

4. **Split / Join pickers (selection-independent).** ✂️ Split and 🔗 Join never use row selection — they are inherently single-client operations. Each opens a searchable picker modal: Split lists every partnered active client and feeds the chosen one into the existing `SplitAssignModal`; Join lists every single (un-partnered) active client and feeds it into the existing `JoinModal`. Opening either picker first exits any active selection mode. No split/join logic is new — the existing modals and their merge math are reused unchanged.

5. **Four new App-level handlers.** `archiveMany(ids)` / `restoreMany(ids)` set the `archived` flag (true / false) across a set of client ids; `deleteMany(ids)` filters them out; `splitClientPair(origId,p1,p2)` removes the original client and inserts its two split halves. `splitClientPair` is a `ClientList`-friendly sibling of the existing `splitClient`, which depends on the App-level `selected` client and therefore stays unchanged for `ClientDetail`'s kebab. `joinClients` is reused as-is for the Join picker. All four new handlers are `useCallback`-wrapped. The per-client kebab inside `ClientDetail` is untouched.

6. **28 new translation keys × 2 languages = 56 entries.** Added to both `T.en` and `T.es`: `selectAllVisible`, `selectedLbl`, `selectToArchive`, `selectToRestore`, `selectToDelete`, `bulkArchiveSel`, `bulkRestoreSel`, `bulkDeleteSel`, `bulkSplitSel`, `bulkJoinSel`, `bulkArchiveTitle`, `bulkArchiveQ`, `bulkRestoreTitle`, `bulkRestoreQ`, `bulkDeleteTitle`, `bulkDeleteQ`, `bulkDeleteTypeHint`, `permanentActionLbl`, `splitPickTitle`, `splitPickHelp`, `joinPickTitle`, `joinPickHelp`, `noPartneredClients`, `noSingleClients`, `noClientsMsg`, `archivedClientsLbl`, `viewLbl`, `restoreLbl`. Dictionary 1,158 → **1,186 per side**; symmetry verified by an `Object.keys(T.en).length === Object.keys(T.es).length` parse.
...
**Build marker:** `2026-05-16-v080-bulk-client-actions`. App.jsx 2,743 → 2,854 lines (~571 → ~591 KB; net +111 lines, almost entirely from `ClientList` being rewritten multi-line). `src/translations.js` 1,158 → 1,186 keys/side (~82 → ~85 KB).
...
- Spot-check: (a) Clients tab — the list looks normal, no checkboxes, row click opens the client as before; (b) ☰ menu → Archive… — an orange action bar appears, checkboxes show on active rows only, archived rows are dimmed; tick 2-3, hit Archive (N), confirm modal lists their names, confirm → they move to Archived; (c) ☰ → Restore… — the Archived section auto-expands, checkboxes show on archived rows only; (d) ☰ → Delete… — checkboxes on every row; the confirm modal's button stays disabled until you type DELETE exactly; (e) ☰ → Split… → pick a partnered client → `SplitAssignModal` opens; ☰ → Join… → pick a single client → `JoinModal` opens; (f) Cancel in any action bar returns the list to its plain state; (g) switch to ES — the menu items, action bars, confirmation modals, and pickers are fully translated.
