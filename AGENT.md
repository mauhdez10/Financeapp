...
**v0.10.1** — established 2026-05-18 (Patch). Ad-hoc bug-fix batch on top of v0.10.0 — **not a queued WORKPLAN chat**. Three fixes:

1. **Supabase client de-duplication (the launch-blocker).** Root cause: `clients` rows created before the v0.5.x `local_id` column have `local_id = NULL`. `gaSaveClient` looked up rows only by `local_id`, so a NULL-`local_id` row never matched → every save INSERTed a *new* duplicate row; `gaDeleteClient` matched only by `local_id` → legacy rows were never soft-deleted (client reappeared on refresh). Two same-`id` rows then loaded into React state, where every id-keyed handler (`upClient`/`archiveClient`/`restoreClient`, React `key`) hit both. Fix: `gaSaveClient` now falls back to matching `data->>'id'` (the JSON blob's own id) when `local_id` misses, and the UPDATE payload backfills `local_id`; `gaDeleteClient` soft-deletes by `local_id` OR `data->>'id'`; `gaLoadClients` de-dupes by client id. **Requires a one-time Supabase cleanup** — run `sql/v0.10.1_clients_local_id_repair.sql` (de-dupe duplicate rows, then backfill NULL `local_id`s). Until that runs, the app code already hides the duplicates and stops creating new ones, but the stale rows still physically exist.
2. **Page-grid sizing regression.** v0.9.0 switched Calculators / Resources / About / Services grids to `auto-fill` with minmax minimums that were too small, packing many tiny columns on desktop. Minimums raised (Calculators 160→240, Resources 220→260, About advisor 260→300, Services 220→250); still collapse to one column on a phone.
3. **Clear Sent Invites.** New `gaDeleteAllIntakeInvites` helper + a "?? Clear all" button on the Intake Forms → Sent invites panel (confirm-gated).

**Build marker:** `2026-05-18-v0101-client-dedup-grids-invites`. App.jsx 2,962 → **2,965 lines**. `src/translations.js` 1,217 → **1,219 keys/side** (+2 × 2 langs: `intakeClearInvites`, `intakeConfirmClearInvites`). New file: `sql/v0.10.1_clients_local_id_repair.sql`. New pitfall #15. No new locked decisions, none closed.

**Out-of-app actions required:**
- **Run the SQL repair**: in Supabase SQL Editor, run `sql/v0.10.1_clients_local_id_repair.sql` — STEP 1 (diagnostic) first, review output, then STEPs 2–3, then STEP 4 to verify (expect 0 / 0).
- **Replace** `src/App.jsx`, `src/translations.js`, `AGENT.md`. **Add** `sql/v0.10.1_clients_local_id_repair.sql`. Append the CHANGELOG entry.
- Commit + push; hard-refresh; verify `window.__GA_BUILD__ === "2026-05-18-v0101-client-dedup-grids-invites"`.
- Separately, the v0.10.0 deploy steps below are still outstanding if intake invites error with "Supabase env vars missing on server" — set the 7 Vercel env vars and run the v0.10.0 SQL migration.
...
15. **Supabase `clients` rows can have a NULL `local_id` (legacy rows).** `local_id` was added in v0.5.x to map app `gid()` ids to Supabase UUIDs (see pitfall #12). Rows created before that have `local_id = NULL`. Any cloud helper that finds a client *only* by `local_id` will silently miss those rows — `gaSaveClient` was INSERTing duplicates, `gaDeleteClient` was failing to delete. **Rule:** client lookup helpers must match by `local_id` OR by the blob's own `data->>'id'`, and any UPDATE should backfill `local_id` so the row self-heals. De-dupe by client id on load so React state never holds two clients with the same `id` (every id-keyed handler and React `key` assumes uniqueness). Fixed in v0.10.1 (code) + `sql/v0.10.1_clients_local_id_repair.sql` (data).
...
*Last updated: 2026-05-18 — v0.10.1 (Patch — ad-hoc bug-fix batch, not a queued chat. (1) Supabase client de-duplication: `gaSaveClient` falls back to `data->>'id'` when `local_id` misses and backfills `local_id` on UPDATE; `gaDeleteClient` matches `local_id` OR `data->>'id'`; `gaLoadClients` de-dupes by client id. Root cause was legacy `clients` rows with NULL `local_id` — saves INSERTed duplicates, deletes silently missed. Paired one-time cleanup `sql/v0.10.1_clients_local_id_repair.sql`. (2) Calculators/Resources/About/Services grid minmax minimums raised — v0.9.0 set them too small, packing tiny columns on desktop. (3) New `gaDeleteAllIntakeInvites` + "Clear all" button on the Sent invites panel. +2 translation keys × 2 langs; 1,217 → 1,219/side. App.jsx 2,962 → 2,965 lines. New pitfall #15. No new/closed locked decisions. D-1, D-7, D-18, D-27 preserved). Build marker `2026-05-18-v0101-client-dedup-grids-invites`.*
