...
**v0.12.4** — established 2026-05-20 (Patch — Email Complete Report PDF section parity + soft-grey background for all three on-screen PrintBtns).

**What shipped:** Two coupled fixes for the v0.12.0 → v0.12.3 PDF/print surface, requested by Mauricio after the v0.12.x PDF smoke test landed (`golden-anchor-report-Amanda-Chen-2026-05-19.pdf` vs `Golden_Anchor_Finance.pdf` comparison).

**Problem 1 — the email PDF was missing data and missing sections.** The `buildPrintHTML()` in `api/render-report-pdf.js` from v0.12.0 was reading the wrong JSON field names:
- Income: read `.amount` but App.jsx writes `.netMo` (gross + frequency-adjusted to monthly), so the email PDF showed $0 monthly income on a client whose live dashboard showed $9,600.
- Debts: read `.interestRate` but App.jsx writes `.apr`, so APR column showed `—`.
- Bills: only read `.name`, missed rows whose label was on `.label`.
- Accounts: aggregated naively, so cash accounts (~$15,800) didn't roll into net-worth; the email PDF showed Net Worth $22,000 vs the live $28,100.

It was also missing five sections present on-screen: Investment Allocation, Financial Ratios (liquidity, debt-to-asset, emergency-fund), Cash Flow Statement, and Strategy Plan (debt payoff order + 2-phase roadmap + 5/10/20-year investment projection).

**Problem 2 — every PDF (email AND in-browser `window.print()`) was pure white, hard to read.** All three on-screen `PrintBtn` flows — Monthly Statement, Financial Statements, Complete Report — go through the global `@media print` CSS, which forced `body { background: white }`. Saved PDFs had no contrast between page and cards.

**The fix:**

1. **`api/render-report-pdf.js` rewritten** (599 → 817 lines): `computeAggregates()` now reads `.netMo` with `.amount` fallback for income, `.apr` for debts, both `.name` and `.label` for bills, and correctly separates cash accounts (checking/savings) for the emergency-fund ratio. All five missing sections built out — Investment Allocation (donut + table reading `client.investAllocation`), Financial Ratios (3 KPI cards + 3-row benchmark table: Liquidity Ratio = cash/bills, Debt-to-Asset = debt/totalAssets×100, Emergency Fund = cash/bills in months), Cash Flow Statement (inflows/outflows grid + operating-cash-flow highlight), Strategy Plan (debt payoff order avalanche table + 2-phase roadmap red/green cards + 3-column investment projection at 8.5%/yr × 5/10/20 years). The PDF now respects `client.reportInclude` toggle map — sections turned off in the on-screen Complete Report won't appear in the PDF. Background of the PDF set to `#F1F5F9` (matches DEF_SETTINGS.lightBg), sections sit on white cards with `#E2E8F0` borders.
2. **App.jsx `@media print` block at line 2963 patched** (one CSS rule changed, ~3 lines added): `body { background: white }` → `html, body { background: #F1F5F9 }`, added `print-color-adjust: exact` (and the WebKit + legacy variants) on `*` so Chrome/Edge/Safari actually preserve the painted backgrounds when they generate the saved PDF — without this rule, browsers strip background colors by default at print time as an ink-saving measure. Applies to **all three on-screen PrintBtns automatically** (MonthlyReportTab line 1382, FinancialStatementReportTab line 1383, CompleteReportTab line 1382-area) since they all share the same `@media print` global. No changes to PrintBtn itself.

**No App.jsx logic changes, no new components, no translation key changes.** The v0.12.3 scope-fix work (D-36) is preserved untouched; the v0.12.2 Spanish keys (1,313/side) are preserved untouched; the `t?.X||"fallback"` belt-and-suspenders sites are preserved untouched. The patch is **one CSS rule + one file rewrite** + build marker.

**Build marker:** `2026-05-20-v0124-section-parity-grey-print`. `src/translations.js` unchanged at 1,313 keys/side. `package.json` unchanged. `vercel.json` unchanged. No SQL migration. No new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31, D-34, D-36 preserved.

**Background color choice (option b from the design conversation, 2026-05-19):** `#F1F5F9` — the same soft cool grey the on-screen light-mode `DEF_SETTINGS.lightBg` already uses. Other options considered + rejected: `#FAF9F7` (too subtle), `#E2E8F0` (too much contrast against white cards, looked more like a glossy-magazine layout than a financial report). Picked the in-app match so the saved PDF and on-screen view feel like the same document.

**Out-of-app actions required:**
- **Replace** `src/App.jsx`, `api/render-report-pdf.js`, `AGENT.md`, `WORKPLAN.md`; append the `CHANGELOG.md` entry. **No** `translations.js` change. **No** `package.json` or `vercel.json` change.
- Commit + push; Vercel auto-deploys. The `api/render-report-pdf.js` bundle does NOT need any new npm install — the v0.12.1 chromium-min runtime, `puppeteer-core@^24.10`, and `@sparticuz/chromium-min@^140` from v0.12.1 are reused as-is.
- Hard-refresh; verify `window.__GA_BUILD__ === "2026-05-20-v0124-section-parity-grey-print"`.
- **Smoke test 1 (email PDF):** open the same Amanda Chen client → Reports → Complete Report → 📧 Email → send to yourself. The PDF should now show Net Worth $28,100, Income $9,600, Bills $1,985, Total Debt $31,700, plus the five new sections (Investment Allocation, Financial Ratios, Cash Flow Statement, Strategy Plan with debt payoff + roadmap + projection). Page background should be soft grey with white section cards.
- **Smoke test 2 (in-browser print/save):** open any client → Reports → Monthly tab → 🖨️ Print / Save PDF → in the browser's print dialog, **enable "Background graphics"** (this is the user-facing checkbox that maps to `print-color-adjust: exact` — most browsers default it OFF). Verify the saved PDF has the soft-grey page background. Repeat for the Financial and Complete tabs.

---

### Prior: v0.12.3 — 2026-05-20 (Patch — Hotfix for v0.12.2; `t`-out-of-scope crash)

Hotfix for a production-breaking bug in v0.12.2. After deploying v0.12.2, the app rendered a **blank screen immediately after login** for every user. Root cause: the v0.12.2 fix script was a regex-based source-text replacement that wired hardcoded English strings through `t.key||"fallback"` without verifying that `t` was actually in scope at each substitution site. **Eight component functions** referenced new `t.xxx` keys inside their bodies but did **not** accept `t` as a parameter:
- `Kebab` (rendered on every dashboard client row — this was the killer; React crashed on the first paint after login)
- `PTag` (client row badges)
- `BulkSnapModal`, `ImportWizard`, `DuplicateResolverModal`, `DeleteClientModal`, `BackupImportModal`, `ExportModal`

At runtime, JavaScript hit `ReferenceError: t is not defined` → React error boundary fired → blank screen. The v0.12.2 TypeScript dry-run never caught this because `t.foo` where `t` is undefined is **valid syntax**; the crash is a runtime `ReferenceError`, not a parse error. The static EN→ES audit and key-symmetry checks didn't catch it either — both were source-text checks, not scope checks.

**The fix:** added `t` as a destructured prop to all eight component signatures and propagated `t={t}` at every JSX call site (22 call sites total). Also added `t` to `ArchivedSection` (dead-code path, but it calls `DeleteClientModal` and could be reached by a future refactor; safer to wire it now). **Belt-and-suspenders pass:** every `t.X||"fallback"` reference newly introduced in v0.12.2 (113 unique dict keys across 177 source sites) was additionally wrapped with optional chaining — `t.X||"fallback"` → `t?.X||"fallback"`. So if a future refactor *again* loses `t` from some scope, the code degrades to the English fallback instead of crashing with `ReferenceError`. Pre-existing `t.X||` patterns (~984 sites) were left untouched.

**The patcher itself was rewritten** to be brace-aware. After patching, a scope-aware static verifier confirms (a) every function that references a known dictionary key has `t` in its parameter list or declares `const t=` itself, and (b) every JSX call site passing `t={t}` has `t` in its enclosing function's scope. Both checks now pass with zero findings.

**Dictionary:** no changes — `translations.js` stayed at v0.12.2's 1,313 keys/side. **App.jsx:** 3,046 → 3,046 lines (signature changes only). **Build marker** `2026-05-20-v0123-t-scope-fix`. **D-36** added (static-text patches MUST be verified by a scope-aware checker, not just a syntax check).

---

### Prior: v0.12.2 — 2026-05-19 (Patch — Spanish bleed-through fix; closes O-9 and O-10; **broken at runtime, hotfixed by v0.12.3**)
...
*Last updated: 2026-05-20 — v0.12.4 (Patch — Email Complete Report PDF section parity + soft-grey background for all three on-screen PrintBtns). Two coupled fixes for the v0.12.x PDF/print surface, requested by Mauricio after the smoke-test comparison of `golden-anchor-report-Amanda-Chen-2026-05-19.pdf` (email) vs `Golden_Anchor_Finance.pdf` (in-browser print/save). Problem 1: the email PDF was reading wrong field names (`.amount` instead of `.netMo` for income, `.interestRate` instead of `.apr` for debts, missed `.label` for bills, didn't roll up cash accounts) so the Net Worth showed $22,000 vs the live $28,100; it also was missing five sections (Investment Allocation, Financial Ratios, Cash Flow Statement, Strategy Plan with debt payoff + roadmap + projection). Problem 2: the global `@media print` rule forced `body { background: white }`, so the in-browser print/save PDFs for Monthly + Financial + Complete had no contrast between page and cards. Fix: `api/render-report-pdf.js` rewritten (599 → 817 lines) — correct field extraction + full section parity + `client.reportInclude` toggle support + `#F1F5F9` page background with white cards; App.jsx `@media print` block patched to `html, body { background: #F1F5F9 }` with `print-color-adjust: exact` on `*` so browsers preserve the painted backgrounds at print time (without this rule, Chrome/Edge/Safari strip background colors by default as an ink-saving measure). No App.jsx logic changes, no new components, no translation key changes. translations.js unchanged at 1,313 keys/side. package.json unchanged. vercel.json unchanged. v0.12.3 scope-fix work (D-36) preserved untouched; v0.12.2 Spanish keys preserved untouched. App.jsx 3,046 → 3,046 lines (one CSS rule edit + build marker). Build marker `2026-05-20-v0124-section-parity-grey-print`. **User-facing note for in-browser print:** Chrome's "Background graphics" checkbox in the print dialog must be checked for the grey to appear in the saved PDF (it's the user-facing toggle that maps to `print-color-adjust: exact`; we set the CSS rule, browsers still ask the user to opt in). No new locked decisions, no new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31, D-34, D-36 preserved.*

*Prior update: 2026-05-20 — v0.12.3 (Patch — Hotfix for v0.12.2 t-out-of-scope crash). v0.12.2 broke production immediately after deploy — blank screen after login, every user. 8 component functions (Kebab/PTag/BulkSnapModal/ImportWizard/DuplicateResolverModal/DeleteClientModal/BackupImportModal/ExportModal) referenced t.xxx without t in their params → ReferenceError → React crash. Mauricio rolled back to v0.12.1. v0.12.3 added t to all 8 signatures + 22 t={t} call sites, with a rewritten brace-aware JSX patcher and a new scope-aware static verifier (both checks now pass zero). Belt-and-suspenders: 177 sites using v0.12.2-introduced keys wrapped with optional chaining (t?.X||"fallback"). New locked decision D-36 (scope-aware checker mandatory for bulk static-text patches). translations.js unchanged. App.jsx 3,046→3,046 lines. Build marker `2026-05-20-v0123-t-scope-fix`. Deploy must include BOTH the v0.12.2 translations.js AND the v0.12.3 App.jsx (since Mauricio rolled back to v0.12.1).*

*Prior update: 2026-05-19 — v0.12.2 (Patch — Spanish bleed-through fix, Chat 8; closes O-9 and O-10). Static EN→ES audit of App.jsx vs translations.js found 134 hardcoded English strings across 203 code sites that never routed through t.key. All 203 wired through `t.key||"fallback"`: 48 reused keys, 83 new keys × 2 langs (neutral Latin-American Spanish — e.g. Down→Enganche, Taxable→Gravable, Checking→Cuenta Corriente). translations.js 1,230→1,313 keys/side, EN/ES symmetry verified. The two O-9 Financial Roadmap PhaseCard/Phase `sub` template literals are now fully bilingual. PrintBtn default param reverted to a plain-string default (a function-parameter default cannot be a JSX expression); all 3 <PrintBtn/> call sites pass a translated label. O-10 Resend production smoke test passed — two real intake invites (EN+ES) to Mauricio's inbox, full open→submit→link-back loop confirmed. App.jsx 3,045→3,046 lines. Build marker `2026-05-19-v0122-spanish-bleedthrough`. No SQL, no env vars, no api/* or package.json changes. No new locked decisions, no new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31, D-34 preserved.*
