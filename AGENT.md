...
**v0.12.0** — established 2026-05-19 (Minor — Email Complete Report as PDF via Puppeteer + Resend).

**What shipped:** The advisor can now email a client their **Complete Report** as a real PDF attachment from inside the Complete Report tab. **O-11 RESOLVED → approach (a) Puppeteer rendering a self-contained print HTML server-side** (NOT driving the live SPA — see D-34 for the rationale). Chat 10 of the parallel-chat workplan.
...
1. **New Vercel Serverless Function** `api/render-report-pdf.js` (a D-30 `api/` server file):
   ...
   - Puppeteer (`puppeteer-core` + `@sparticuz/chromium`) renders the HTML to PDF at Letter size, 0.5in margins.
...
4. **`vercel.json`** picks up a `functions` block — `api/render-report-pdf.js` configured with `memory: 1024` and `maxDuration: 30` (Puppeteer cold start ~2–4s, warm ~1s).
5. **`package.json`** adds two runtime deps — `puppeteer-core@^23.0.0` and `@sparticuz/chromium@^131.0.0`. `resend` is already there from v0.10.0.
6. **One new locked decision D-34** (PDF rendering = print HTML + Puppeteer, NOT driving the live SPA). **O-11 CLOSED.** **O-13 CLOSED** (PDF generation for the Complete Report is now in place; manual `window.print()` stays for in-browser save-as-PDF).

**Email signature pattern:** same as v0.11.1 — pulled from Profile & Settings (`advisorName` / `advisorEmail` passed through in the payload), with a fallback to the historical defaults. Independent of `RESEND_FROM`.

**Build marker:** `2026-05-19-v0120-email-report-pdf`. No SQL migration. No new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31 preserved.
...
- **D-34 — PDF report rendering = self-contained print HTML + Puppeteer, NOT driving the live SPA (v0.12.0, 2026-05-19; closes O-11 and O-13).** When a server function needs to produce a PDF of a Complete Report (or any future report), it MUST render a self-contained HTML document built server-side from the client data (no React, no SPA boot, no advisor login), then Puppeteer that HTML to PDF. Concretely: `puppeteer-core` + `@sparticuz/chromium` for the slim Chromium bundle (full `puppeteer` exceeds Vercel's function size limit); function memory 1024 MB and `maxDuration: 30` in `vercel.json`; cold start ~2–4s, warm ~1s. Charts are hand-rolled inline SVG in the Golden Anchor palette — no Recharts, no chart library in the function bundle. **Reasoning** (decided 2026-05-19 with Mauricio): driving `finance.goldenanchor.life` in a headless browser would require injecting a Supabase session into the page context (refresh token exposure), wait for Recharts animations + chart layout to settle, and silently break on any App.jsx layout drift. Print HTML is faster, more stable, smaller cold-start surface, and the auth model stays clean. **Maintenance cost:** any Complete Report visual change must be mirrored in `buildPrintHTML()` inside `api/render-report-pdf.js`. For v1 the Complete Report is mostly tabular (income / bills / debt / assets / notes), so mirroring is cheap. **Delivery:** Resend email attachment (base64-encoded PDF). Considered + rejected for v1: upload to Supabase Storage bucket `client-reports` + signed-URL link in the email — adds storage surface and a "log in / click a link" step for the client; revisit if Resend's per-email size limit becomes a problem at scale (current PDFs are 100–500 KB, well under). **Forward note:** when v0.13+ adds monthly-statement PDFs or annual-summary PDFs, they extend `api/render-report-pdf.js` (or add a sibling `api/render-monthly-pdf.js`) under the same pattern; do NOT re-open the print-HTML-vs-SPA-driving question without surfacing this decision.
...
*Last updated: 2026-05-19 — v0.12.0 (Minor — Email Complete Report as PDF, Chat 10). New Vercel function `api/render-report-pdf.js` (D-30 server file) verifies the advisor JWT, loads the client row via service-role + two `.eq()` calls (pitfall #15 avoided), builds a self-contained printable HTML document with inline SVG donut + bar charts in the Golden Anchor palette, Puppeteer (`puppeteer-core` + `@sparticuz/chromium`) renders it to PDF, Resend (D-31) attaches and emails. App.jsx 2,998 → 3,046 lines: new `gaEmailCompleteReport` helper, new `EmailReportModal` component (recipient auto-fills `client.email`, advisor can override; EN/ES defaults), `CompleteReportTab` gains `settings` prop + 📧 Email button beside `PrintBtn`; `ClientReport` + `ClientDetail` call sites forward `settings`. `translations.js` 1,218 → 1,230 keys/side (12 new × 2 langs, symmetry verified). New `vercel.json` `functions` block (memory 1024, maxDuration 30). New `package.json` deps `puppeteer-core@^23` + `@sparticuz/chromium@^131`. One new locked decision **D-34** (print HTML + Puppeteer, NOT live-SPA-drive). **O-11 closed, O-13 closed.** Build marker `2026-05-19-v0120-email-report-pdf`. No SQL migration. No new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31 preserved.*
