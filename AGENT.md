...
| **UI UX Pro Max plugin** | Already used (v0.26.0 was a UI/UX Pro Max audit batch, v0.42-v0.45 chart polish followed its alignment notes, v0.43.0 persisted `design-system/golden-anchor/MASTER.md` via `--persist`). **Layer 2 of the design pipeline** — 161 palettes, 57 font pairings, 50+ styles. |
| **Impeccable** skill | **Design pipeline Layer 1** — anti-slop vocabulary + 23 commands (polish/audit/critique/bolder/quieter/animate/typeset/colorize…) + deterministic CLI gate `npx impeccable detect src/`. Run the gate before every design version bump. |
| **Emil (`emil-design-eng`)** skill | **Design pipeline Layer 4** — motion/micro-interactions: <300ms, custom easing, loading states, "review this animation." Never animate high-frequency actions. |
| **21st.dev Magic** MCP (`magic`) | **Design pipeline Layer 3** — component sourcing. `/ui`, `/21` slash cmds → real component code; `21st_magic_component_builder` / `_inspiration` / `_refiner` / `logo_search`. API key in `~/.claude.json` plaintext (rotate before launch). |
| **`design-principles`** skill | **Design pipeline Layer 0** — process + project context, dual-mode discipline, give variations. Triggers on any UI/visual/brand work. |
| **🎨 DESIGN MODE** | When the task is visual/UI/UX, read **`docs/DESIGN-MODE.md`** first. Owner's standing rule (2026-06-05): in design mode, quality is the only objective — credit/time are not constraints. Use EVERY pipeline layer, explore all options in each tool, compare overlapping tools, dual-mode always. |
...
*Prior update: 2026-05-20 — v0.12.4 (Patch — Email Complete Report PDF section parity + soft-grey background). Added 5 missing sections to email PDF. `body { background: white }` → `html, body { background: #F1F5F9 }` + `print-color-adjust: exact` on `*`. Build marker `2026-05-20-v0124-section-parity-grey-print`. Data-extraction code used wrong field names — fixed in v0.12.5. @media print change reverted in v0.12.5 — restored in v0.12.6.*

*Prior update: 2026-05-20 — v0.12.3 (Hotfix for v0.12.2 t-out-of-scope crash). New locked decision D-36 (scope-aware checker mandatory for bulk static-text patches). Build marker `2026-05-20-v0123-t-scope-fix`.*
