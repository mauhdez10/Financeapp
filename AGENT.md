...
**v0.8.1** — established 2026-05-16 (Patch). Customizable page-background and card-background colors in Profile & Settings, for both light and dark mode, with a live preview. A small follow-up to v0.8.0, requested by Mauricio directly (not a queued workplan chat) to fix the low-contrast "white card on near-white page" look in light mode — the advisor can now tune it himself.

1. **Four new settings keys.** `DEF_SETTINGS` gains `darkBg`, `darkCard`, `lightBg`, `lightCard`. Defaults are the exact current palette values (`lightBg:#F1F5F9`, `lightCard:#FFFFFF`, `darkBg:#111827`, `darkCard:#1F2937`), so the app looks identical out of the box — nothing changes visually until the advisor picks new colors. These are properties **inside** the existing `ga-settings` object; **no new top-level localStorage key** is introduced, so D-10 is satisfied without needing a confirmation.

2. **Theme construction applies the overrides.** The single theme line in `App()` previously did `const theme=isDark?makeDark(...):makeLight(...)`. It now builds `_baseTh` from `makeDark`/`makeLight` as before, then spreads it into `theme` with `bg` and `card` overridden from the per-mode settings (`bg:(isDark?settings.darkBg:settings.lightBg)||_baseTh.bg`, same for `card`). Only `bg` (page background) and `card` (card surface) are overridable — `nav`, `modal`, `inp`, `cardBorder`, `accent`, text colors, etc. are untouched, deliberately (the scope Mauricio chose was background + cards only; letting the advisor freely set text colors risks unreadable combinations). The `??`-style `|| _baseTh.x` fallback means a missing or blank setting harmlessly reverts to the palette default.

3. **New "Background Colors" section in `ProfileModal`.** Sits directly under the existing "THEME COLORS" accent rows, non-collapsible (it's part of the theme area and meant to be experimented with). One card per mode (☀️ Light / 🌙 Dark); each card has two pickers — Page background and Card background — plus a live **Preview** tile and a per-mode "↺ Reset" button that restores that mode's two defaults. The preview tile renders the chosen page-bg with a mock card (chosen card-bg) and brand text inside, updating live from the in-progress modal state `s` — so the advisor sees the combination before pressing Save; on Save it applies app-wide via the normal `setSettings` path.

4. **`BgPicker` helper + four preset arrays.** A small `BgPicker` component is defined inside `ProfileModal` (next to the existing `AccRow`): preset shade swatches, a native `<input type="color">` custom picker (guarded by a 6-hex regex so a mid-edit hex string doesn't break the color input), and a monospace hex text field. Four top-level preset arrays were added next to `DARK_ACCENTS`/`LIGHT_ACCENTS`: `LIGHT_BG_PRESETS`, `LIGHT_CARD_PRESETS`, `DARK_BG_PRESETS`, `DARK_CARD_PRESETS` — plain hex-string arrays, pure data.

5. **6 new translation keys × 2 languages = 12 entries.** Added to both `T.en` and `T.es`: `appearanceHdr`, `appearanceHelp`, `pageBackgroundLbl`, `cardBackgroundLbl`, `resetToDefault`, `previewLbl`. Mode labels reuse the existing `lightMode` / `darkMode` keys. Dictionary 1,186 → **1,192 per side**; symmetry verified.

No new locked decisions, none closed, no new pitfalls. D-1, D-7, D-10, D-18, D-29 preserved. No SQL migration — the settings blob already round-trips to Supabase as JSONB (D-19), so the four new keys persist automatically. v0.8.1 contains everything from v0.8.0 (bulk client actions) on top of v0.7.3; if v0.8.0 was not separately committed, committing v0.8.1 ships both.

**Build marker:** `2026-05-16-v081-appearance-bg-settings`. App.jsx 2,854 → 2,856 lines (~591 → ~595 KB). `src/translations.js` 1,186 → 1,192 keys/side (~85 KB).
...
- Add CHANGELOG.md entries for v0.8.0 and v0.8.1 (see chat delivery for Option A instructions) if v0.8.0 was not already logged.
