...
**v0.12.5** — established 2026-05-20 (Patch — Email PDF data extraction CORRECTLY fixed + Email button on Monthly + Financial tabs + modal backdrop-close disabled).

**What shipped:** Three fixes in one patch, addressing the issues that surfaced after the v0.12.4 deploy + smoke test:

**Fix 1 — Email PDF data extraction (CORRECTLY this time).** v0.12.4 was supposed to make the email PDF show the same numbers as the live app, but the fix script invented field names that **don't exist** on the actual stored data:
- I wrote `.amount` / `.netMo` for income; the real fields are `.net` + `.freq` (per-period, with `toM(net, freq)` converting to monthly).
- I wrote `.amount` for bills; the real field is `.cost` + `.freq` (also `toM`-converted).
- I wrote `client.debts[].minPayment`; the real path is `client.cards[]` (NOT `.debts`!) + a derived `effectiveMin(c)` function (cards with `.min === 0` get a default computed from balance × 1% + monthly interest, floored at $25).
- I wrote `client.accounts[].balance`; the real field is `.value`. Cash accounts are filtered by `ACCT_META.liquid` (checking, savings, money_market — NOT retirement).
- Loans were missed entirely; the real `totalL(client)` adds both `client.cards` AND `client.loans` balances.
- Investment allocation: I wrote `client.investAllocation` with a nested `{checked, pct, label}` shape; the real path is `client.alloc = { stocks: 25, retirement: 20, ... }` (key→pct) paired with `client.committed = { stocks: true, ... }` (key→boolean).

Result in production: the v0.12.4 email PDF for the Amanda Chen test client showed `Net Worth $22,000` (was supposed to be `$28,100`), `Income $0` (was `$9,600`), `Bills $0` (was `$1,985`), `Total Debt $0` (was `$31,700`). The only number that came through was `$22,000` from the custom Fidelity Account, because `.value` happens to be the right field there.

**v0.12.5 fix:** `api/render-report-pdf.js` helpers + `computeAggregates()` block (lines 70-167, ~100 lines) completely rewritten to **mirror the live `App.jsx` math exactly**, with citations back to the App.jsx source lines: `FREQ` table at line 121, `toM` at line 122, `actB` at line 129, `cardMoInt` at line 136, `effectiveMin` at line 133, `sumN`/`sumB`/`sumMin`/`liquidA`/`totalA`/`totalL` from lines 130-145, `ACCT_META` at line 58. Every wrong field reference inside `buildPrintHTML` (10 separate sites in income tables, bill tables, debt tables, account tables, investment allocation, debt payoff) was surgically corrected. Cards-and-loans tagged at construction (`_isCard: true|false`) so the min-pay column can pick `effectiveMin(c)` for cards vs raw `.min` for loans.

**Fix 2 — Email button on Monthly Report + Financial Statements tabs.** v0.12.0 added the 📧 Email button only to the Complete Report tab. The Monthly Report and Financial Statements tabs only had 🖨️ Print / Save PDF. v0.12.5 adds the email button next to PrintBtn on both tabs (wrapped in a `<div className="ga-np">` so both buttons hide during in-browser print). Both tabs now accept a `settings` prop and forward it to `EmailReportModal`. The `ClientReport` call sites pass `settings={settings}` to both new locations. All three Email buttons send the same Complete Report PDF for now — per-tab differentiation (Monthly-style vs Financial-style PDF) is deferred to Chat 11.

**Fix 3 — Email modal no longer closes on backdrop click.** Mauricio reported accidental loss of typed content when clicking outside the modal mid-edit. v0.12.5 adds a `disableBackdropClose` prop to the shared `Modal` component (defaults to `false` — preserves every other modal's existing dismiss-on-click-out behavior). Only `EmailReportModal` opts in. Closing the email modal now requires the explicit Cancel button or the × icon.

**D-36 scope-aware check:** the 3 new `<EmailReportModal client={client} lang={lang} t={t} settings={settings} onClose={...}/>` call sites all reference variables that exist in their enclosing function scopes (`MonthlyReportTab`, `FinancialStatementReportTab`, `CompleteReportTab`). `Btn`, `EmailReportModal`, and `PrintBtn` are all top-level function declarations and hence module-scoped, so the function-hoisting handles forward references cleanly (`EmailReportModal` is declared at line 643 but referenced from line 641 — fine in JS).

**Build marker:** `2026-05-20-v0125-email-on-all-reports-data-fix`. App.jsx 3,046 → 3,046 lines (no line additions; the two tab changes were one-line replacements). `src/translations.js` unchanged at 1,313 keys/side (the `emailReportBtn` key already exists from v0.12.0). `package.json` and `vercel.json` unchanged. No SQL migration. No new locked decisions. No new pitfalls.

**Out-of-app actions required:**
- **Replace** `src/App.jsx`, `api/render-report-pdf.js`, `AGENT.md`, `WORKPLAN.md`; append `CHANGELOG.md` entry. **No** changes to `translations.js`, `package.json`, `vercel.json`.
- Commit + push; Vercel auto-deploys.
- Hard-refresh; verify `window.__GA_BUILD__ === "2026-05-20-v0125-email-on-all-reports-data-fix"`.
- **Smoke test 1 (Complete Report email):** same Amanda Chen client → Reports → Complete Report → 📧 Email. PDF should now show **Income $9,600, Bills $1,985, Total Debt $31,700, Net Worth $28,100** (was $0/$0/$0/$22,000 in v0.12.4). Five sections should render with correct numbers — Investment Allocation should show Stocks 25%, Retirement 20%, etc. with non-zero dollar amounts.
- **Smoke test 2 (Email button on Monthly + Financial tabs):** open Reports → Monthly Report → confirm 📧 Email button appears beside 🖨️ Print / Save PDF. Click it, send to yourself. Confirm same Complete Report PDF arrives. Repeat for Financial Statements tab.
- **Smoke test 3 (modal backdrop):** open Reports → Complete Report → 📧 Email → click anywhere on the dark backdrop *outside* the modal. Modal should NOT close. Click Cancel or × — those should close it.

---

### Prior: v0.12.4 — 2026-05-20 (Patch — Email Complete Report PDF section parity + soft-grey print background)

Two coupled fixes for the v0.12.x PDF/print surface, requested by Mauricio after the smoke-test comparison of `golden-anchor-report-Amanda-Chen-2026-05-19.pdf` (email) vs `Golden_Anchor_Finance.pdf` (in-browser print/save). **Problem 1 — email PDF was missing 5 sections** (Investment Allocation, Financial Ratios, Cash Flow Statement, Strategy Plan). **Problem 2 — every PDF was pure white, hard to read** — the global `@media print` rule forced `body { background: white }`. **Fix:** (1) `api/render-report-pdf.js` rewritten (599 → 817 lines) to render the 5 missing sections + soft-grey `#F1F5F9` page background with white section cards. (2) App.jsx `@media print` block at line 2963 patched: `body { background: white }` → `html, body { background: #F1F5F9 }` + `print-color-adjust: exact` on `*`. **Build marker** `2026-05-20-v0124-section-parity-grey-print`. **However**, the data-extraction part of v0.12.4 used wrong field names — the email PDF still rendered with all zeros despite the new sections. This is what v0.12.5 corrects.

### Prior: v0.12.3 — 2026-05-20 (Patch — Hotfix for v0.12.2; `t`-out-of-scope crash)

Hotfix for a production-breaking bug in v0.12.2. After deploying v0.12.2, the app rendered a **blank screen immediately after login** for every user. Root cause: 8 component functions referenced new `t.xxx` keys inside their bodies but did not accept `t` as a parameter (Kebab, PTag, BulkSnapModal, ImportWizard, DuplicateResolverModal, DeleteClientModal, BackupImportModal, ExportModal). JavaScript hit `ReferenceError: t is not defined` → React error boundary → blank screen. **Fix:** added `t` as a destructured prop to all 8 signatures, propagated `t={t}` at 22 call sites; rewrote the patcher to be brace-aware; added a scope-aware static verifier (both checks now pass zero). Belt-and-suspenders: every `t.X||"fallback"` reference newly introduced in v0.12.2 (113 unique dict keys / 177 sites) wrapped with optional chaining. `translations.js` unchanged at 1,313 keys/side. **Build marker** `2026-05-20-v0123-t-scope-fix`. **D-36 added** (static-text patches MUST be verified by a scope-aware checker, not just a syntax check).
...
*Last updated: 2026-05-20 — v0.12.5 (Patch — Email PDF data CORRECTLY fixed + Email button on Monthly + Financial tabs + modal backdrop-close disabled). Three fixes in one patch responding to Mauricio's smoke-test feedback on v0.12.4. (1) The v0.12.4 email-PDF data fix used wrong field names everywhere — `.amount`/`.netMo` for income (actual: `.net`+`.freq` via `toM`), `.amount` for bills (actual: `.cost`+`.freq` via `toM`), `client.debts` (actual: `client.cards` + `client.loans`), `.minPayment` (actual: `effectiveMin(c)` for cards / `.min` for loans), `.balance` for accounts (actual: `.value`), `client.investAllocation` with nested `{checked,pct,label}` (actual: `client.alloc` + `client.committed`). Result: v0.12.4 PDF showed Net Worth $22,000 instead of $28,100, Income $0 instead of $9,600. v0.12.5 rewrites the helpers + `computeAggregates` block (~100 lines) to mirror the live App.jsx math exactly with citations back to source lines (FREQ@121, toM@122, actB@129, effectiveMin@133, etc.), and surgically corrects 10 wrong field references throughout `buildPrintHTML`. (2) Email button added to Monthly Report tab + Financial Statements tab (wrapped in `<div className="ga-np">` so they hide during in-browser print). Both tabs now accept `settings` prop and forward to EmailReportModal; ClientReport call sites updated. All three buttons send the same Complete Report PDF for now (per-tab differentiation deferred to Chat 11). (3) Modal gains `disableBackdropClose` prop (defaults to false, preserving every other modal's behavior); EmailReportModal opts in, so accidental clicks outside the modal no longer dismiss it mid-typing. App.jsx 3,046 → 3,046 lines (one-line replacements only). render-report-pdf.js 817 → 885 lines. `translations.js` unchanged at 1,313 keys/side. `package.json`, `vercel.json` unchanged. Build marker `2026-05-20-v0125-email-on-all-reports-data-fix`. D-36 scope check passed (no new function references undeclared variables). No SQL migration. No new locked decisions, no new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31, D-34, D-36 preserved.*

*Prior update: 2026-05-20 — v0.12.4 (Patch — Email Complete Report PDF section parity + soft-grey background for all three on-screen PrintBtns). Added 5 missing sections (Investment Allocation, Financial Ratios, Cash Flow Statement, Strategy Plan) to the email PDF. Changed `body { background: white }` → `html, body { background: #F1F5F9 }` with `print-color-adjust: exact` for all PrintBtn flows. **However**, the data-extraction part of v0.12.4 used wrong field names — the email PDF still rendered with all zeros despite the new sections. This is what v0.12.5 corrects. Build marker `2026-05-20-v0124-section-parity-grey-print`.*

*Prior: 2026-05-20 — v0.12.3 (Patch — Hotfix for v0.12.2 t-out-of-scope crash). v0.12.2 shipped Friday and crashed every login with a blank screen. 8 components referenced new t.xxx keys without t in scope → ReferenceError → blank screen. Fix: added t to all 8 signatures + 22 t={t} call sites + scope-aware verifier. Belt-and-suspenders: 177 sites using v0.12.2-introduced keys wrapped with optional chaining. New locked decision D-36 (scope-aware checker mandatory for bulk static-text patches). translations.js 1,313 keys/side. Build marker `2026-05-20-v0123-t-scope-fix`.*
