...
**v0.11.0** — established 2026-05-19 (Chat 9 — Browser history / back-button integration).

**What shipped:** The advisor app is a single-page app; tab/section navigation (`nav`, `selected`, `selectedTab`) was plain React state with no browser-history integration, so the browser accumulated no in-app entries and pressing Back left the app entirely (landed before login). v0.11.0 wires `history.pushState`/`popstate`:

1. **One navigation-signature effect.** A single `useEffect` keyed on `[nav, selected?.id, selectedTab, authUser, isPublicIntakeRoute]` pushes a history entry whenever the in-app navigation signature changes — instead of instrumenting each `setNav`/`setSelected`/`setSelectedTab` call site (~10 of them). The first run does `replaceState` (seeds the entry point); later runs `pushState`. The state object is `{ga:true, nav, selectedId, selectedTab}`.
2. **`popstate` listener.** Reads the state object, restores `nav` / `selectedTab` / `selected` (client looked up by id from `clients`). A no-state `popstate` (user backed out past the app's first entry) falls back to the dashboard instead of unloading the SPA.
3. **Re-push guard.** A `_popstateRestoringRef` ref suppresses the push effect on the render caused by a `popstate` restore, so Back/Forward don't spawn new entries (no loop, no double entries).
4. **Mobile drawer.** If the off-canvas drawer is open, the first Back closes the drawer (and re-pushes the same entry) rather than navigating.
5. **Pitfall #13 honored.** Both new effects sit in the effects block, below every hook declaration and above the `isPublicIntakeRoute` early return. The push effect early-returns on the `/intake` public route and while signed out, so history is only tracked inside the authenticated app.

**Auth-gate interaction:** unchanged and confirmed safe — the `!authUser` early return renders `<Login/>` regardless of `nav`, so a signed-out `popstate` cannot land "inside" the app; and the push effect's `if(!authUser)return` means no in-app entries are created before login.

**Out of scope (unchanged):** deep-linkable URLs (`/clients/<id>` paths) — Back/Forward work, but URLs are not shareable. No new features, no translation work.

**Build marker:** `2026-05-19-v0110-history-backbutton`. App.jsx 2,965 → **3,001 lines**. `src/translations.js` unchanged (no new strings). New pitfall #16. No new locked decisions, none closed.

**Out-of-app actions required:**
- **Replace** `src/App.jsx`, `AGENT.md`, `WORKPLAN.md`; append the `CHANGELOG.md` entry. No `translations.js` change.
- Commit + push; hard-refresh; verify `window.__GA_BUILD__ === "2026-05-19-v0110-history-backbutton"`, then test Back/Forward across dashboard ↔ client ↔ tabs, and (on a phone) that Back closes an open drawer first.

---

### Prior: v0.10.2 — 2026-05-18 (Patch — client delete fix)

One-line code fix on top of v0.10.1. **`gaDeleteClient` rewritten.** v0.10.1 had soft-deleted via a single `.or("local_id.eq.${id},data->>id.eq.${id}")` filter; PostgREST parses an `or` filter by splitting the string on `.`, so JSON paths (`data->>id`) and decimal client ids (e.g. `1776873994030.0803`, a real legacy id) corrupt the query → 400 → the soft-delete silently no-ops → the client reappears on refresh. Archive was unaffected (it routes through `gaSaveClient`, which uses `.eq()`). Fixed with two plain `.eq()` UPDATE calls (`local_id`, then `data->>id` fallback). **Correction to the v0.10.1 record:** its stated root cause ("legacy rows with NULL `local_id`") was wrong — live data showed `local_id` populated on every row; `sql/v0.10.1_clients_local_id_repair.sql` was a no-op and was correctly never committed. The "client duplication" launch-blocker was also investigated against live data and found to be **cross-account confusion** (the same `local_id` value `1` exists under both Mauricio's account and the Playwright test account, with different client names) — there were no real duplicate rows within a single account. v0.10.1's grid / invites / `gaLoadClients` de-dupe / `gaSaveClient` blob-id fallback changes remain valid. Build marker `2026-05-18-v0102-delete-fix`.
...
- **D-31 — Email provider = Resend, verified on `finance.goldenanchor.life` (v0.10.0, 2026-05-18; refines D-20).** DNS records (SPF/DKIM/DMARC) are live on Cloudflare and Resend dashboard shows the domain verified. Sender: `mauricio@finance.goldenanchor.life` (configurable via `RESEND_FROM` env). Reply-to: `mauricio@goldenanchor.life` (so prospect replies route to Google Workspace, not Resend). Inbound email handling at Resend stays disabled — Chat 7 only sends outbound. 3,000 emails/month free tier; if Mauricio exceeds it, upgrade to Resend Pro ($20/mo for 50k emails). Forward note: when Phase-2 features (engagement letters, monthly reports, payment receipts) start sending more transactional email, monitor volume.
...
- **O-11 — PDF generation approach for *report* email automation.** *Scope clarified v0.10.0:* this is exclusively about emailing the **Complete Report** (Dashboard / ClientReport) to a paying client — NOT about the intake invite flow. Intake invites are link-only (a prospect filling out an empty form), so they don't need PDF generation. PDF generation only becomes a blocker when the future "email Complete Report to client" feature lands. Three options still open when that chat starts: (a) Puppeteer/Playwright on a Vercel function — renders the real React page to PDF server-side, most faithful, heaviest cold start; (b) `@react-pdf/renderer` rebuild — duplicates report layout but produces clean PDFs fast, worst maintenance burden; (c) `jspdf` + `html2canvas` client-side — generate PDF in browser, upload to Supabase Storage, attach to email, no server work but uneven quality (rasterizes charts, fonts get weird). Defer pick until that chat is queued.
...
- **O-13 — PDF generation timing for launch.** Decided v0.5.2a: **deferred to post-launch**. *Updated v0.10.0:* Resend email automation IS now active (v0.10.0 ships Resend for intake invites) but the invite flow doesn't need a PDF — it's link-only. PDF generation specifically targets the *Complete Report* delivery feature, which is still post-launch. Current `window.print()` remains fine for the manual report-share flow. Track O-11 for the implementation choice when the time comes.
...
15. **Never put a JSON path or a dotted value inside a PostgREST `.or()` filter.** supabase-js `.or("col.op.val,col.op.val")` is parsed by splitting the string on `.`, so a JSON path like `data->>id` — and any value containing `.`, such as the decimal client ids in legacy data (`1776873994030.0803`) — corrupt the filter and the request 400s. v0.10.1's `gaDeleteClient` hit exactly this and silently failed every delete (the client reappeared on refresh). **Rule:** to match on `local_id` and/or `data->>'id'`, use separate `.eq()` calls — `.eq()` URL-encodes the whole value, so dots are safe. For app-written rows `local_id` is reliably `String(client.id)` (written by `gaSaveClient` and queried by `gaDeleteClient` with the same operation), so `.eq("local_id", String(client.id))` always matches; a second `.eq("data->>id", …)` pass covers any row whose `local_id` is unset. `gaLoadClients` still de-dupes by client id on load so React state never holds two same-`id` clients. Fixed in v0.10.2. NOTE: v0.10.1 framed this pitfall as "NULL `local_id` legacy rows" — that root cause was wrong (`local_id` is populated on every row); the real bug was the `.or()` parsing described here.
16. **In-app navigation must push browser history or Back unloads the SPA.** Tab/section navigation is plain React state (`nav`, `selected`, `selectedTab`); without `history.pushState` the browser keeps no in-app entries and the Back button leaves the app. Fixed in v0.11.0 with a navigation-signature `useEffect` (push on `[nav, selected?.id, selectedTab]` change) plus a `popstate` listener. **Rule:** the push effect must run after a first-render `replaceState` to seed the entry point; a `_popstateRestoringRef` guard must suppress the push on the render caused by a `popstate` restore, or Back/Forward spawn duplicate entries; and — per pitfall #13 — both effects belong in the effects block below every hook and above the `isPublicIntakeRoute` early return, with the push effect early-returning on the `/intake` route and while signed out.
...
*Last updated: 2026-05-19 — v0.11.0 (Chat 9 — Browser history / back-button integration). In-app navigation now pushes browser history: a navigation-signature `useEffect` calls `pushState` on `[nav, selected?.id, selectedTab]` changes (first run `replaceState` to seed), a `popstate` listener restores nav/selected/tab, a `_popstateRestoringRef` guard prevents re-push loops, and an open mobile drawer is closed by the first Back. Both effects sit below all hooks and above the `isPublicIntakeRoute` early return (pitfall #13). translations.js unchanged. App.jsx 2,965 → 3,001 lines. New pitfall #16. No new/closed locked decisions. D-1, D-7, D-18, D-27 preserved. Build marker `2026-05-19-v0110-history-backbutton`. Prior: v0.10.2 (Patch) — `gaDeleteClient` `.or()` → two `.eq()` delete fix; corrected the v0.10.1 NULL-`local_id` misdiagnosis; "client duplication" found to be cross-account test data, not a real bug.)*
