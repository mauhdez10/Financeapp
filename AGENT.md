...
**v0.12.3** — established 2026-05-20 (Patch — Hotfix for v0.12.2; `t`-out-of-scope crash).

**What shipped:** Hotfix for a production-breaking bug in v0.12.2. After deploying v0.12.2, the app rendered a **blank screen immediately after login** for every user. Root cause: the v0.12.2 fix script was a regex-based source-text replacement that wired hardcoded English strings through `t.key||"fallback"` without verifying that `t` was actually in scope at each substitution site. **Eight component functions** referenced new `t.xxx` keys inside their bodies but did **not** accept `t` as a parameter:
...
**The fix:** added `t` as a destructured prop to all eight component signatures and propagated `t={t}` at every JSX call site (22 call sites total). Also added `t` to `ArchivedSection` (dead-code path, but it calls `DeleteClientModal` and could be reached by a future refactor; safer to wire it now).

**Belt-and-suspenders pass:** every `t.X||"fallback"` reference newly introduced in v0.12.2 (113 unique dict keys across 177 source sites) was additionally wrapped with optional chaining — `t.X||"fallback"` → `t?.X||"fallback"`. So if a future refactor *again* loses `t` from some scope, the code degrades to the English fallback instead of crashing with `ReferenceError`. Pre-existing `t.X||` patterns (~984 sites) were left untouched — they've been battle-tested by months of production traffic, and changing them would balloon the diff without reducing risk.

**The patcher itself was rewritten** to be brace-aware — the first attempt used a naive `<Name[^>]*?/?>` regex that mistakenly inserted `t={t}` inside JSX expression-attribute bodies (e.g. `onClose={()=>` was matched as the end of the tag at the first `>`, so `t={t}` was injected mid-arrow-function, producing `onClose={()=> t={t}>setRestoreOpen(false)}` — also broken at runtime). The corrected patcher walks the tag character-by-character tracking brace depth (and string-quote state) until it finds the real closing `>` or `/>` at depth 0. After patching, a scope-aware static verifier confirms (a) every function that references a known dictionary key has `t` in its parameter list or declares `const t=` itself, and (b) every JSX call site passing `t={t}` has `t` in its enclosing function's scope. Both checks now pass with zero findings.

**Dictionary:** no changes — `translations.js` stays at v0.12.2's 1,313 keys/side. **App.jsx:** 3,046 → 3,046 lines (signature changes only, no line additions). **Build marker** `2026-05-20-v0123-t-scope-fix`. No SQL migration. No new locked decisions. **No new pitfalls** beyond what's documented in §4 (D-36 added below).

**Out-of-app actions required:** **emergency redeploy.** Replace `src/App.jsx`, `AGENT.md`, `WORKPLAN.md`; append the `CHANGELOG.md` entry. No `translations.js` change (Mauricio rolled back to v0.12.1 to recover the app, so the deploy must include both the v0.12.2 `translations.js` (with the 83 new keys) AND the v0.12.3 `App.jsx`). Commit + push; Vercel auto-deploys. Hard-refresh; verify `window.__GA_BUILD__ === "2026-05-20-v0123-t-scope-fix"`. Test by logging in and confirming the dashboard renders (no blank screen). Then toggle to Spanish and confirm calculators/client-data modals/Backfill flow still read fully in Spanish.

**D-36 added in §4:** future static-text patches must be verified by a **scope-aware checker**, not just a syntax check.
...
*Last updated: 2026-05-20 — v0.12.3 (Patch — Hotfix for v0.12.2 t-out-of-scope crash). v0.12.2 shipped Friday and crashed every login with a blank screen. Root cause: 8 component functions (Kebab, PTag, BulkSnapModal, ImportWizard, DuplicateResolverModal, DeleteClientModal, BackupImportModal, ExportModal) referenced new t.xxx keys in their bodies but did not accept t as a parameter — JavaScript threw ReferenceError at render time; React error boundary fired; blank screen. Kebab was the killer because it renders on every dashboard client row (immediate crash on the first paint after login). Mauricio rolled back to v0.12.1 to recover the app. v0.12.3 fix: added t to all 8 signatures, propagated t={t} at all 22 call sites; also added t to ArchivedSection (dead-code path, defense-in-depth). The patcher was rewritten brace-aware: a naive `<Name[^>]*?/?>` regex used in the first round inserted t={t} inside JSX expression-attribute bodies (e.g. mid `onClose={()=>setX(false)}`), producing different broken syntax; the corrected patcher walks characters tracking { } depth and string-quote state until the real depth-0 closing > or />. A new scope-aware static verifier confirms (a) every function referencing a known dict key has t in scope, (b) every t={t} call site has t in its enclosing scope — both report zero. Belt-and-suspenders: every t.X|| reference newly introduced in v0.12.2 (113 unique keys / 177 sites) was wrapped with optional chaining (t?.X||"fallback") so a future scope regression degrades to the fallback string instead of ReferenceError — pre-existing t.X|| sites left untouched (battle-tested). translations.js unchanged (still 1,313 keys/side from v0.12.2). App.jsx 3,046→3,046 lines (signatures only). TypeScript dry-run clean. Build marker `2026-05-20-v0123-t-scope-fix`. New locked decision D-36 (static-text patches MUST be verified by a scope-aware checker, not just a syntax check). No SQL, no env vars, no api/* or package.json changes. D-1, D-7, D-18, D-27, D-30, D-31, D-34 preserved. Deploy must include BOTH the v0.12.2 translations.js AND the v0.12.3 App.jsx (since Mauricio rolled back to v0.12.1).*
