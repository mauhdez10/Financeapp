...
The actual app is a **single React file**: `src/App.jsx` (~2,254 lines, ~590 KB as of v0.5.1 / 2026-05-14).
...
**v0.5.1** — established 2026-05-14. **Critical Supabase migration bug fix.** The v0.5.0 wiring used `clientObj.id` (a numeric value from `gid() = Date.now() + Math.floor(Math.random()*99999)`) as the primary key when upserting into Supabase's `clients` table, but the `clients.id` column is a UUID. Every save failed silently with a PostgreSQL cast error; the migration loop marked `ga_migrated_to_supabase = "1"` anyway, leaving the app stuck reading from localStorage with no path to retry.
...
**Required Supabase SQL** (paste into Supabase SQL Editor and run before deploying):
...
2. Confirm the version you're loading (currently v0.5.1).
...
*Last updated: 2026-05-14 — v0.5.1 (Patch — critical Supabase migration bug fix). The v0.5.0 wiring tried to insert app `gid()` numeric IDs into Supabase's UUID `clients.id` column, which silently failed for every save while the migration loop set its done-flag anyway, locking the app into half-migrated state with clients still living in localStorage. Three-part fix: (1) `gaSaveClient` now uses a new `local_id` text column with select-then-update-or-insert pattern, returns success boolean; (2) `gaDeleteClient` matches rows by `local_id`; (3) `gaMigrateLocalStorage` only sets `ga_migrated_to_supabase=1` if every save succeeds, otherwise retries on next login. Requires one SQL statement in Supabase SQL Editor (see §3) before deploy, plus `localStorage.removeItem("ga_migrated_to_supabase")` once in browser console after deploy. New pitfall #12 added documenting the UUID-vs-numeric-ID class of bug. Build marker bumped to `2026-05-14-localid-migration-v051`. Next active work: Mauricio runs the SQL, deploys, runs the localStorage reset, verifies clients land in Supabase via the SQL query in CHANGELOG v0.5.1 entry.*
