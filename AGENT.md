...
**v0.10.2** — established 2026-05-18 (Patch). One-line code fix on top of v0.10.1 — **not a queued WORKPLAN chat**.

**The fix:** `gaDeleteClient` rewritten. v0.10.1 had used a single `.or("local_id.eq.${id},data->>id.eq.${id}")` filter to soft-delete a client. That is broken: PostgREST parses an `or` filter by splitting the string on `.`, so a JSON path (`data->>id`) and any value containing a `.` (decimal client ids such as `1776873994030.0803` — a real legacy id, see pitfall #15) corrupt the query → 400 error → the soft-delete silently no-ops → the client reappears on refresh. Archive was unaffected because it routes through `gaSaveClient`, which uses `.eq()`. v0.10.2 replaces the `.or()` with two plain `.eq()` UPDATE calls — one matching `local_id`, one matching `data->>id` as a fallback. `.eq()` URL-encodes the whole value, so dots are safe and no `or`-string parsing is involved.

**Correction to the v0.10.1 record.** v0.10.1's stated root cause — "legacy `clients` rows with `local_id = NULL`" — was wrong. Inspection of live data showed `local_id` populated on every row. The NULL-`local_id` cleanup script (`sql/v0.10.1_clients_local_id_repair.sql`) was therefore a no-op and was never committed (correctly). v0.10.1's other changes — grid sizing, Clear-all invites, `gaLoadClients` de-dupe, `gaSaveClient` blob-id fallback — are harmless and stay in place; only its `gaDeleteClient` change was defective and is now superseded.

**Build marker:** `2026-05-18-v0102-delete-fix`. App.jsx line count unchanged. `src/translations.js` unchanged (no UI strings). No new files committed. Pitfall #15 amended. No new locked decisions, none closed.
...
- Commit + push; hard-refresh; verify `window.__GA_BUILD__ === "2026-05-18-v0102-delete-fix"`.
- **Optional one-time DB hygiene** (run in Supabase SQL Editor; no need to commit — it is a maintenance script, not app code): a diagnostic query for app-level client duplication, plus a defensive rebuild of the `clients (user_id, local_id) WHERE deleted_at IS NULL` unique index. Provided in the chat that shipped v0.10.2.
- **Intake-invite email** is still not working but the cause has moved on: the old "Supabase env vars missing on server" error is resolved; the current "API key is not authorized to send emails from goldenanchor.life" is a **Resend** configuration issue, not a code bug — the `from` address (`RESEND_FROM`) must use a Resend-verified domain (per D-31, `finance.goldenanchor.life`), and that domain's SPF/DKIM DNS records must be verified in Cloudflare.
...
*Last updated: 2026-05-18 — v0.10.2 (Patch — not a queued chat. `gaDeleteClient` rewritten: v0.10.1's single `.or("local_id.eq…,data->>id.eq…")` filter is broken because PostgREST splits `or` strings on `.`, corrupting JSON paths and decimal client ids — every delete silently 400'd and the client reappeared on refresh. Replaced with two plain `.eq()` UPDATE calls. Also corrected the record: v0.10.1's "NULL `local_id`" root cause was wrong (`local_id` is populated on every row); v0.10.1's grid / invites / de-dupe changes stand, only its `gaDeleteClient` was defective. translations.js unchanged. Pitfall #15 amended. No new/closed locked decisions. D-1, D-7, D-18, D-27 preserved). Build marker `2026-05-18-v0102-delete-fix`.*
