...
**v0.6.2** — established 2026-05-15 (Patch). Pure mechanical refactor — no behavior change, no UI change, no new features, no bug fixes. Single concern: extract the `T.en` / `T.es` translation dictionaries out of `App.jsx` into a sibling `src/translations.js` so future Spanish-translation audit chats can load the dictionary alone (~80 KB) instead of all of App.jsx (~635 KB).
