...
**v0.11.1** — established 2026-05-19 (Patch — intake-invite email signature from Profile & Settings).

**What shipped:** The intake-invite email's signature (advisor name + contact email in the message body) was hard-coded to `Mauricio Hernandez` / `mauricio@goldenanchor.life` inside the server function `api/send-intake-invite.js`. It now comes from the advisor's Profile & Settings:

1. **App.jsx** (2,965 → 2,998 lines, includes the v0.10.2 + v0.11.0 carry-forward below): `IntakeSubmissionsPage` now receives the `settings` prop, and the `gaSendIntakeInvite` payload carries `advisorName` and `advisorEmail` from `settings`.
2. **`api/send-intake-invite.js`** (a D-30 `api/` server file — edited this chat): `buildEmailBody` takes an `advisor` argument; the signature block (both EN/ES, text + HTML) uses `advName` / `advEmail`, falling back to the historical `Mauricio Hernandez` / `mauricio@goldenanchor.life` when the payload omits them. The handler parses `advisorName` / `advisorEmail` from the request body. Also updated the `RESEND_FROM` / `RESEND_REPLY_TO` doc comments to the new `noreply@` address (see D-31).

**To set the signature:** edit **Email** in Profile & Settings (e.g. `finance@goldenanchor.life`) and save — that address, and the **Advisor Name**, are what appear in the invite email's signature. This is independent of `RESEND_FROM` (the technical sender, `noreply@finance.goldenanchor.life`).

**Build marker:** `2026-05-19-v0111-invite-signature`. `src/translations.js` unchanged. No new pitfalls. D-31 amended (sender/reply-to → `noreply@finance.goldenanchor.life`; signature now settings-driven). No other locked decisions changed.

**Out-of-app actions required:**
- **Replace** `src/App.jsx`, `api/send-intake-invite.js`, `AGENT.md`, `WORKPLAN.md`; append the `CHANGELOG.md` entry. No `translations.js` change.
- Confirm the Vercel env vars `RESEND_FROM` and `RESEND_REPLY_TO` are both set to the `noreply@finance.goldenanchor.life` address.
- In the app: open Profile & Settings, set **Email** to the address that should appear in the signature, save.
- Commit + push; hard-refresh; verify `window.__GA_BUILD__ === "2026-05-19-v0111-invite-signature"`; send a test invite to yourself and check the signature.
