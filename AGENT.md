...
**v0.9.0** — established 2026-05-16 (Minor). Mobile / responsive redesign of every primary surface. No data-shape changes, no behavior changes, no new components — purely a layout-primitives pass so the whole app is usable one-handed at a ~360px viewport. Chat 5 of the parallel-chat workplan. Builds on v0.8.1's appearance settings and v0.8.0's bulk client actions; the new bulk-action bar and the new Appearance section both got mobile-tuned in the same pass.

1. **Mobile top bar drops the ⚓.** The sticky app bar that shows on `vp.isMobile` lost the `<span>⚓</span>` so the page title (or selected client name) has more room and never truncates at ~360px. The desktop sidebar keeps its full `⚓ Golden Anchor` brand row — this only changes the mobile bar.

2. **Dashboard KPI grid collapses 2-up on mobile.** The KPI block was `repeat(auto-fit,minmax(140px,1fr))` on every viewport, which on a phone produces a tight single column with no breathing room. It now reads `isMobile?"repeat(2,1fr)":"repeat(auto-fit,minmax(140px,1fr))"`. Six KPIs become a tidy 3×2 on a phone instead of a long ribbon. The donut-charts row beneath the KPIs also stacks (`flex-direction:column`) on mobile. The dashboard header (title + 4 action buttons) wraps, the title font drops a size step, and the search input goes full-width on mobile.

3. **Dashboard client row restructure on mobile.** Each row in the "Active Clients" panel previously laid name+email and two snapshot tiles and net/mo and a `›` chevron in one horizontal strip. On mobile it now stacks: name+email row on top, then snapshot tiles + net/mo below in a wrapped row, and the `›` chevron hides (the whole row is still tap-target). Desktop layout unchanged.

4. **ClientList (Chat 4 bulk-action surface) goes mobile-tuned.** Wired `useViewport()` in. The header search + 📥/⋮ overflow + the always-visible "+ Add" button were a single row on every viewport; on mobile, search becomes full-width, the action buttons drop below it, and "+ Add" grows to share the row. The bulk-action coloured bar (Chat 4 — appears once Archive/Restore/Delete mode is active) now `flex-wrap`s on mobile so the action label, the selected-count, and the Cancel/Apply buttons all stay readable instead of horizontally clipping. Active and archived client rows in the list use the same name-on-top, figures-below stacked pattern as the Dashboard row.

5. **ClientDetail mobile pass.** Wired `useViewport()` in. Outer padding scaled (24 → 12 on mobile). The 4-up KPI row at the top of the client report becomes 2-up on a phone. The charts row stacks vertically. The horizontal tab strip (8 tabs: 📊 Report / 📅 Monthly / 📋 Financials / 💹 Investments / 📋 Plan / 🧮 Calculators / 🔧 Backfill / 🗒 Notes) on mobile gets `overflowX:auto` so it scrolls inside its own region instead of forcing the whole page to scroll sideways — the most common pre-v0.9.0 mobile complaint.

6. **Other surfaces re-fluidified.** `CalculatorsPage` (was fixed grid of 7) → `repeat(auto-fill,minmax(160px,1fr))`. `ResourcesPage` → `repeat(auto-fill,minmax(220px,1fr))`. `AboutPage`'s advisor-card and services grids → auto-fit so they don't push past the viewport. The Appearance section's per-mode preview tile (v0.8.1) is `flex:"0 0 auto"`, the BgPicker column is `flex:"1 1 200px"` with `minWidth:0`, and the preview tile dropped 128 → 120 px so the pair fits beside each other at narrow widths without horizontal scroll. The app's main content column got `maxWidth:"100%"` as a final overflow-safety belt.

7. **Zero new translation keys.** This is a layout-only pass; nothing user-facing was added that needs wording. `src/translations.js` is unchanged (1,192 keys per side). D-18 trivially satisfied.

No new locked decisions, none reversed. D-1 (single-file), D-7 (state lives in `App` only), D-10 (localStorage keys), D-18 (EN/ES symmetry), D-19 (settings blob → Supabase JSONB), and D-27 (mobile-first + PWA direction) are all preserved — D-27 is meaningfully advanced. No SQL migration. No new pitfalls. Chat 4's bulk-action flow and Chat 8.1's Appearance section both included in the audit, per the WORKPLAN Chat 5 spec addendum.

**Build marker:** `2026-05-16-v090-mobile-responsive`. App.jsx 2,856 → **2,858 lines** (~595 → ~589 KB after compaction of some inline styles; net byte count moved up slightly because new conditional branches added ~3 KB while a few duplicated style fragments collapsed). `src/translations.js` unchanged at 1,192 keys/side (~85 KB).

**Out-of-app actions required before this build runs in production:**
- Replace `src/App.jsx` only (translations.js unchanged this build — do not re-upload).
- Replace `AGENT.md` and `WORKPLAN.md` at repo root.
- Add CHANGELOG.md entry for v0.9.0 (see chat delivery for Option A instructions).
- Commit + push. Vercel auto-deploys.
- Hard refresh production; verify `window.__GA_BUILD__ === "2026-05-16-v081-appearance-bg-settings"`.
- Spot-check: (a) open Profile & Settings — a "Background Colors" section appears under Theme Colors with a Light card and a Dark card; (b) in the Light card, click a greyer Page-background swatch — the Preview tile updates immediately and the card-on-page contrast is visible; (c) press Save — the whole app's light-mode page background changes; (d) toggle to dark mode (sidebar) — the dark-mode colors you set are in effect; (e) press ↺ Reset on a mode — its two colors return to default; (f) refresh the page — the chosen colors persist; (g) switch to ES — the section header, help text, picker labels, Preview label, and Reset button are translated.
