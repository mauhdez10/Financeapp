...
**Tooling addendum (2026-05-14, post-v0.5.2a deploy):** Playwright end-to-end test harness added to the repo. 30 tests covering smoke, calculators, client workflows, translation integrity, and Supabase persistence. Test user `test@goldenanchor.life` (UUID `9d017248-fc0a-44ad-b68b-53315bb928d8`) seeded with duplicated fake/demo clients. Main advisor account `b373dd8a-bf12-4df2-9439-d7770406d416` is protected by a hard-refuse guard in `global-setup.ts`. WebKit project disabled in the local Codespace because 36 system libraries are missing — Chromium + Firefox give adequate coverage. First run had ~30 selector-mismatch failures in `02-calculators.spec.ts`, `04-translation.spec.ts`, and the `navTo` chain in `utils/fixtures.ts` — test-code bugs, NOT app bugs. **Selector fixes attempted twice on 2026-05-14:** first attempt landed but still had three real bugs (broken `switchLang` toggle detection, wrong Clients-page placeholder in `openClient`, `getByLabel` against unpaired labels in calculator inputs); second attempt rewrote `utils/fixtures.ts` from scratch using verified App.jsx invariants (toggle by `button[title="Language"]` + `window.__GA_LANG` confirmation, `data-cf` attribute for inputs, tolerant placeholder regex). Expected steady state now 60/60 passing. Detailed in §13. Not a launch blocker; v0.5.2a is in production and working.
...
**First selector-fix attempt (2026-05-14, later same day):** Claimed ~60/60 passing in a CHANGELOG entry, but a re-run of `rm -rf playwright/.auth && npm run test:e2e` still showed 36 failures. The diagnosis at end of that session correctly identified three remaining bugs but the fix was never landed.

**Second selector-fix attempt (2026-05-14, third pass):** Rewrote `utils/fixtures.ts` from scratch keyed to verified App.jsx invariants. Three bugs patched: (a) `switchLang` now locates the toggle by `button[title="Language"]` and confirms the switch via `window.__GA_LANG` + nav re-render, rather than relying on toggle text that doesn't flip; (b) `openClient` waits on a regex matching the actual Clients-page placeholder `🔍 Search…` (hardcoded English at App.jsx:1988, NOT translated — different from the Dashboard's translated `🔍 Search clients…`); (c) added `fillNumberByLabel` using `[data-cf*="..."] input` because the `Field` UI atom at App.jsx:157 renders the `<label>` as a sibling of the `<input>`, not paired via `htmlFor`, so `getByLabel()` cannot work. Expected steady state now 60/60 passing on chromium + firefox. If anything still fails, share the report and we'll triage individually.
...
*Last updated: 2026-05-14 — v0.5.2a (Patch — launch stabilization part 1) + tooling addendum (Playwright) + tooling sub-addendum (Playwright selector fixes, first pass) + tooling sub-addendum (Playwright selector fixes, third pass — actually working). The patch shipped four launch-readiness items: 30-min idle auto-logout with 1-min warning + draft preservation, Mauricio-only password reset via Supabase email flow (no public signup), save-failure toast surfaced from `ga-save-failed` CustomEvent dispatched by `gaSaveClient`/`gaSaveSettings`. 17 new bilingual translation keys (1,060 per side). No schema changes, no client-data shape changes — App.jsx only. New open decisions O-12 (auto-logout config, locked at 30/1 min), O-13 (PDF deferred to post-launch), O-14 (ToS/engagement letter flow deferred to v0.6+), O-15 (Supabase PITR + manual verification, column-level encryption deferred). Build marker bumped to `2026-05-14-autologout-passreset-v052a`. After deploy: Playwright test harness added (5 spec files, 30 tests, dedicated `test@goldenanchor.life` test user, main advisor account protected by hard-refuse guard). WebKit disabled locally due to missing system libs. Initial run surfaced ~30 selector-mismatch failures across `02-calculators.spec.ts`, `04-translation.spec.ts`, and the `navTo` chain — test-code bugs only, not app bugs. First selector-fix attempt the same day claimed ~60/60 but actually still had 36 failures (`switchLang` not firing, wrong Clients-page placeholder in `openClient`, `getByLabel` against unpaired labels in calculators). Third-pass rewrite of `utils/fixtures.ts` keyed to verified App.jsx invariants — toggle by `button[title="Language"]` + `window.__GA_LANG` confirmation, `data-cf` attribute for inputs via new `fillNumberByLabel`, tolerant search placeholder regex `/🔍\s*(Search|Buscar)/i`. Expected steady state now 60/60 passing. See §13. Next: v0.5.2b adds service plan tracking, Stripe Payment Link fields, About/Services Pay Now buttons, and `lastBackupVerified` settings field.*
