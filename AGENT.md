...
**v0.7.0** — established 2026-05-16 (Minor — IA breaking change). Information-architecture refactor: standalone Forms tab deleted, per-client Intake tab merged into the Monthly Statement, Investment Allocation + Emergency Fund moved out of the client-facing intake surface and locked into the advisor-only Monthly Statement. No schema change, no migration.

1. **`FormsPage` deleted; standalone Forms tab removed from NAV.** The `{id:"forms",l:"📋 "+t.forms}` entry is gone from the top-level `NAV` array, the `nav==="forms"?<FormsPage t={t}/>:` render branch is gone from `App()`, and the `FormsPage` component itself (one-line, ~1.3 KB) is removed. Its sole helper `dlTmpl` (the CSV template generator) is also deleted as orphan. Net effect: the 📋 Forms entry no longer appears in the sidebar; the CSV template download path is gone (Mauricio confirmed it was unused).

2. **"Intake Submissions" renamed → "Intake Forms".** EN `intakeSubmissions` value: `"Intake Submissions"` → `"Intake Forms"`. ES: `"Formularios Recibidos"` → `"Formularios de Admisión"`. Inline EN fallbacks at the `IntakeSubmissionsPage` header (L2299) and the NAV label (L2527) updated to match. The `intakeShareUrlHelp` string updated in both languages to reference the new queue name. The page component itself keeps the name `IntakeSubmissionsPage` (internal identifier; user-facing wording comes from `t.intakeSubmissions`).

3. **Per-client Intake tab removed from `ClientDetail`.** The `{id:"intake",l:"📝 "+t.intake}` tab entry and the corresponding `{tab==="intake"&&<IntakeSection .../>}` render branch are gone. The `IntakeSection` component **definition is retained** (line 449, ~1.3 KB) per spec — Mauricio's plan is to wire it into the global Intake Forms surface later. As of v0.7.0 it is defined but never instantiated; this is intentional (flagged as a deliberate orphan-but-retained, **not** a candidate for v0.7.0 cleanup).

4. **Investment Allocation + Emergency Fund moved out of intake, exclusive to Monthly Statement.** `SavingsSection` (the single component holding both the Emergency Fund progress bar and the Investment Allocation pie + per-bucket table) is no longer rendered inside `IntakeSection`. It continues to render in `MonthlyTab` via `FullMonthView` for both the current-month tabbed view (`sec==="savings"`) and the full-page report (`fullPage===true`) and historical month view (`HistView` → `FullMonthView` with `fullPage===true`). The Complete Report's Investment Allocation block renders unchanged. Net effect: clients filling intake never see these fields; the advisor sets them inside Monthly Statement → 💰 Savings (or via the full-page report).

5. **`setSelectedTab("intake")` redirects → `"monthly"`.** Two call sites updated: (a) the post-Convert handler in `IntakeSubmissionsPage` that lands the advisor on the converted client's detail (now opens Monthly Statement instead of the deleted Intake tab); (b) the `addClient` callback (post-`New Client` modal) which now also opens Monthly Statement. Both flows previously assumed the per-client Intake tab existed.

6. **Per-client `client.intakeData` not migrated.** Per spec, Mauricio confirmed existing per-client intake data is test data only. Audit confirms the field `client.intakeData` does **not exist anywhere** in App.jsx (no read, no write, no migration in `mig()`); the intake surface previously wrote to the same client root fields (`firstName`, `incomeStreams`, `bills`, `cards`, etc.) that every other editor uses, plus the current-month snapshot. So "discard intake test data on read" is a no-op for code — nothing to remove.

7. **Six orphan translation keys deleted × 2 languages = 12 entries.** Removed from both `T.en` and `T.es`: `forms`, `formsTitle`, `formsDesc`, `downloadCSVTemplate`, `howToUseColon`, `newClientOnboarding`. All six were used only inside `FormsPage`. Dictionary 1,147 → **1,141 per side**, symmetry verified (`Object.keys(T.en).length === Object.keys(T.es).length`, zero asymmetric keys). One pre-existing orphan in translations.js (`newClientOnboardingDesc`) was not touched — it predates this chat and is out of scope for the Forms cleanup.

No new locked decisions, none closed, no new pitfalls. D-1, D-7, D-18, D-29 preserved.

**Build marker:** `2026-05-16-v070-ia-refactor-intake-forms`. App.jsx 2,565 lines (~548 KB), down 3 from v0.6.3 (FormsPage + dlTmpl removed; small inline additions for the redirect changes net out). `src/translations.js` ~79 KB (down from ~79 KB — six key entries × 2 langs removed, intakeShareUrlHelp values lengthened slightly in both langs).
...
- Add a CHANGELOG.md entry for v0.7.0 (see CHANGELOG.md update notes below or in the chat that shipped this).
- Commit + push (see terminal commands in the chat delivery). Vercel auto-deploys.
- Hard refresh production; verify `window.__GA_BUILD__ === "2026-05-16-v070-ia-refactor-intake-forms"`.
- Spot-check: (a) confirm 📋 Forms is gone from the sidebar; (b) confirm the 📥 nav entry now reads "Intake Forms" (EN) / "Formularios de Admisión" (ES); (c) open a client → confirm the 📝 Intake tab is gone and the tab strip no longer wraps; (d) open Monthly Statement → 💰 Savings → confirm Emergency Fund + Investment Allocation are still there and editable; (e) submit a test intake via the public URL → convert it → confirm the new client opens on Monthly Statement (not the now-deleted Intake tab).
- Run Playwright: expect failures in `03-client-workflows.spec.ts` (any selector that opens the Intake tab) and `04-translation.spec.ts` (the "Intake Submissions" string check). These are anticipated and will be fixed in Chat 6 (Playwright resync). Do **not** treat them as app bugs.
...
*Last updated: 2026-05-16 — v0.7.0 (Minor — IA refactor: standalone Forms tab + `FormsPage` deleted, per-client Intake tab merged into Monthly Statement, Investment Allocation + Emergency Fund moved exclusively to advisor-only Monthly Statement, "Intake Submissions" renamed → "Intake Forms" EN + "Formularios de Admisión" ES, six orphan translation keys removed per side). Build marker `2026-05-16-v070-ia-refactor-intake-forms`. Dictionary 1,147 → 1,141 per side, symmetry intact. App.jsx 2,568 → 2,565 lines. No SQL migration. No new locked decisions, no closed open decisions. No new pitfalls.*

*Prior update: 2026-05-16 — v0.6.3 (Patch — Service Plan editor trimmed to 4 fields, Pay Now/Pay Later buttons, clientGoals to second person). Build marker 2026-05-16-v063-service-plan-trim-notes-tone.*
