...
**v0.9.3** — established 2026-05-17 (Patch). Mobile KPI/portfolio grids that were hard-coded to 3 or 4 columns and clipping cards off the right edge of the viewport — surfaced by Mauricio's three screenshots in this session (ClientReport `Net Worth` cut off; FinancialStatements `Current Ratio 0.82x` cut off; Investments `Aggressive` package card cut off). v0.9.0's pass missed these because the mobile branches were only added to the components I touched directly; this patch adds a defensive global rule plus targeted tags so any future hard-coded grid is auto-collapsed on mobile.

1. **Global mobile-grid-collapse rule injected into `ga-styles`.** A new `@media(max-width:719px)` block targets `[data-ga-grid="kpi"]`, `[data-ga-grid="kpi-3"]`, `[data-ga-grid="kpi-4"]`, `[data-ga-grid="portfolios"]`, and `[data-ga-grid="two-col"]` and forces them to `repeat(2,minmax(0,1fr))` (KPI rows) or `1fr` (portfolio/two-col rows) with `!important`. Inline styles still win at the cascade level, so the `!important` is required to override `gridTemplateColumns:"repeat(4,1fr)"` baked into the JSX. There's also a `.ga-mobile-collapse` class for ad-hoc cases.

2. **`SC` (stat card) component now has `className="ga-sc"`, `minWidth:0`, `overflow:hidden`.** The CSS rule `.ga-sc,.ga-sc *{min-width:0!important}` lets the cards actually shrink inside a collapsed grid; without it, grid items default to `min-width:auto` (≈ content width) and refuse to compress past their natural size — that's why the v0.9.0 pass on grids that already had `repeat(auto-fit,minmax(140px,1fr))` still spilled occasionally on very long values. Labels and values inside SC also got `overflow:hidden;text-overflow:ellipsis;white-space:nowrap` so a $999,999,999 amount truncates with `…` instead of expanding the card.

3. **Four problem grids tagged with `data-ga-grid`.** `ClientReport` 4-up KPI strip (Net Income/Bills/Debt/Net Worth) → `kpi-4`. `AssetsLiabilitiesTab` 4-up (Total Assets/Liab/Net Worth/Current Ratio) → `kpi-4`. `RatioContent` 3-up KPI → `kpi-3`. `InvestmentsTab` and `PortfolioStandaloneCalc` Conservative/Growth/Aggressive 3-card rows → `portfolios` (collapses to single column on mobile so the editable rate input + risk pill stay readable). Portfolio cards also got `minWidth:0,overflow:hidden` on each inline card style.

4. **Pattern is forward-safe.** Future grid additions just need to slap `data-ga-grid="kpi-4"` (or `kpi-3` / `portfolios` / `two-col`) on the wrapping `<div>` and they'll auto-collapse on mobile without any layout logic at the JS level. Cheaper than wiring `useViewport()` into every component. The remaining hard-coded `repeat(4,1fr)` / `1fr 1fr 1fr` grids inside specific calculators (RetirementCalc scenarios, etc.) and a couple of summary rows are left untagged for now — they're either inside modals that have their own narrow scrolling or inside content that's only seen on desktop in current workflows; if any of them get reported in another screenshot the fix is one-line.

No new locked decisions, none closed, no new pitfalls (this is a re-application of v0.9.0's intent that scope-missed the screen surfaces above — not a new category of bug). D-1 (single-file), D-7 (state in `App` only), D-18 (zero new strings), D-27 (mobile-first) preserved. No SQL migration. v0.9.3 contains all of v0.9.0 / v0.9.1 / v0.9.2.

**Build marker:** `2026-05-17-v093-mobile-grid-overflow`. App.jsx 2,880 → **2,900 lines** (~595 KB). `src/translations.js` unchanged at 1,192 keys/side.
...
- CHANGELOG.md still optional per Mauricio's decision this session.
- Commit + push. Vercel auto-deploys. Verify on phone: open ClientDetail → Financial Statements; Total Assets/Liab/Net Worth/Current Ratio should be a 2×2 grid, all four values fully visible. Open Investments → Portfolio; Conservative/Growth/Aggressive cards should stack vertically.
