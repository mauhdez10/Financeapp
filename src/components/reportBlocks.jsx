...
export function PlanReportBlock({client,lang,t}){
  ...
  return<div style={{marginBottom:14}}>
    ...
    {totalDebt===0&&net>0&&<div style={{...mCARD(th),padding:14,marginBottom:14,background:th.pos+"11",border:`1px solid ${th.pos}33`,fontSize:13,fontWeight:700,color:th.pos}}>🎉 {t?.debtFreeFocus||"Debt Free — focus on building wealth."}</div>}
    ...
    {totalDebt>0&&<div style={{...mCARD(th),padding:16,marginBottom:14}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>{phaseSorted.map((d,i)=>{const ev=payEvents.find(e=>e.id===d.id);return<div key={d.id} style={{...mCARD(th),padding:"10px 14px",display:"flex",alignItems:"center",gap:12}}><div style={{width:24,height:24,borderRadius:99,background:th.accent,color:"#fff",fontWeight:800,fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{i+1}</div><div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:th.text}}>{d.name}</div><div style={{fontSize:11,color:th.dim}}>{fmt(d.balance)} · {d.apr}% APR · {(t?.minPerMo||"Min {amt}/mo").replace("{amt}",fmt(d.min))}</div></div>{ev&&<div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:700,color:th.pos}}>{fmtDur(ev.month)}</div><div style={{fontSize:10,color:th.dim}}>{addDate(ev.month)}</div></div>}</div>;})}</div>
    </div>}
    {/* CARD 3 — FINANCIAL ROADMAP (phases) */}
    <div style={{...mCARD(th),padding:16,marginBottom:14}}>
      <div style={{fontSize:12,fontWeight:700,color:th.dim,marginBottom:10}}>🗺️ {t?.financialRoadmapHdr||"FINANCIAL ROADMAP"}</div>
      {totalDebt>0&&<PhaseCard num={1} color={th.neg} title={t.payOffAllDebt||"Pay Off All Debt"} sub={(t?.applyingExtraSub||"Applying {amt}/mo extra to fastest debt using {strat}").replace("{amt}",fmt(extra)).replace("{strat}",strat==="avalanche"?(t?.avalanche||"Avalanche"):(t?.snowball||"Snowball"))} badge={{val:fmtDur(debtMonths),label:addDate(debtMonths)}} note={ov.phase1||(t?.phase1NoteDefault||"Focus all extra cash on debt. Projected payoff in {dur}. Avoid new debt during this phase.").replace("{dur}",fmtDur(debtMonths))}/>}
      {efGap>0&&<PhaseCard num={totalDebt>0?2:1} color={th.warn} title={t.buildEmerFund||"Build Emergency Fund"} sub={(t?.phase2Sub||"Need {amt} more · {n}-month target").replace("{amt}",fmt(efGap)).replace("{n}",client.efMonths||3)} badge={{val:fmtDur(debtMonths+efMonths),label:addDate(debtMonths+efMonths)}} note={ov.phase2||(t?.phase2NoteDefault||"After debt is gone, redirect payments to savings. Target {n} months of expenses ({amt}). Keep in HYSA.").replace("{n}",client.efMonths||3).replace("{amt}",fmt(efTarget))}/>}
      <PhaseCard num={totalDebt>0&&efGap>0?3:totalDebt>0||efGap>0?2:1} color={th.pos} title={t.investBuildWealth||"Invest & Build Wealth"} sub={(t?.phase3Sub||"~{amt}/mo to investments ({pct}% allocation from your plan). Est. 8.5% avg return").replace("{amt}",fmt(investPerMo)).replace("{pct}",(investPct*100).toFixed(0))} badge={{val:"🚀",label:investStart===0?(t?.startNow||"Start now"):addDate(investStart)}} note={ov.phase3||(t?.phase3NoteDefault||"Allocate {s}% stocks + {r}% retirement. Dollar-cost average monthly. Max employer 401k match first.").replace("{s}",alloc.stocks||0).replace("{r}",alloc.retirement||0)}/>
    </div>
    {/* CARD 4 — INVESTMENT PROJECTION (already self-contained mCARD) */}
    {investPerMo>0&&<div style={{...mCARD(th),padding:14,marginBottom:14}}><div style={{fontSize:11,fontWeight:700,color:th.dim,marginBottom:10}}>📈 {t.investmentProjectionHdr||"INVESTMENT PROJECTION"} · {t?.startsLbl||"starts"} {addDate(investStart)} · {(investPct*100).toFixed(0)}% {t?.ofExtraCash||"of extra cash"}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>{[[5,t?.fivePctYears||"5 Years"],[10,t?.tenYears||"10 Years"],[20,t?.twentyYears||"20 Years"]].map(([yrs,label])=><div key={yrs} style={{textAlign:"center"}}><div style={{fontSize:10,color:th.dim,marginBottom:4}}>{label}</div><div style={{fontSize:18,fontWeight:800,color:th.pos}}>{fmt(investFV(yrs))}</div><div style={{fontSize:10,color:th.dim}}>+{fmt(investFV(yrs)-investPerMo*yrs*12)} {t?.growthSuffix||"growth"}</div></div>)}</div></div>}
    {/* CARD 5 — Additional Notes (if any) */}
    {ov.extra&&<div style={{...mCARD(th),padding:16,marginBottom:14}}>
      <div style={{fontSize:12,fontWeight:700,color:th.accent,marginBottom:6}}>📝 {t?.additionalNotesRecsHdr||"Additional Notes / Recommendations"}</div>
