...
export function ClientList({clients,t,onSelect,loadClientBlob,onAdd,onRestore,onImportNew,onRestoreBackup,onArchiveMany,onRestoreMany,onDeleteMany,onSplit,onJoin}){
  ...
  // v0.83 — sort/display from SUMMARY fields (advisor rows are lightweight summaries, not blobs).
  const _sortFn=(a,b)=>{
    if(sortBy==="debt"){return(+b.total_debt||0)-(+a.total_debt||0);}
    if(sortBy==="income"){return(+b.monthly_income||0)-(+a.monthly_income||0);}
    if(sortBy==="recent"){return(+new Date(b.last_activity||0))-(+new Date(a.last_activity||0));}
    if(sortBy==="netWorth"){return(+b.net_worth||0)-(+a.net_worth||0);}
