...
/* ── CALCULATORS ─────────────────────────────────────────────────────────── */
function AmortTablePaginated({data,t={}}){const th=useTh();const[page,setPage]=useState(1);const per=10;const total=data.length;const start=(page-1)*per;const visible=data.slice(start,start+per);return<><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr><th style={mTH(th)}>{t.tblYear||"Year"}</th><th style={mTHR(th)}>{t.amortColBalance||"Balance"}</th><th style={mTHR(th)}>{t.amortColPaidInt||"Paid Interest"}</th><th style={mTHR(th)}>{t.amortColPaidPrin||"Paid Principal"}</th></tr></thead><tbody>{visible.map(r=><tr key={r.yr}><td style={mTD(th)}>{t.yrPrefix||"Yr"} {r.yr}</td><td style={{...mTDR(th),color:th.neg}}>{fmt(r.bal)}</td><td style={{...mTDR(th),color:th.warn}}>{fmt(r.totInt)}</td><td style={{...mTDR(th),color:th.pos}}>{fmt(r.totPrin)}</td></tr>)}</tbody></table></div><Paginator total={total} page={page} setPage={setPage} perPage={per}/></>;}

function EquityTablePaginated({data,t={}}){const th=useTh();const[page,setPage]=useState(1);const per=10;const total=data.length;const start=(page-1)*per;const visible=data.slice(start,start+per);return<><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr><th style={mTH(th)}>{t.tblYear||"Year"}</th><th style={mTHR(th)}>{t.equityColHome||"Home Value"}</th><th style={mTHR(th)}>{t.equityColMortgage||"Mortgage"}</th><th style={mTHR(th)}>{t.equityColEquity||"Equity"}</th></tr></thead><tbody>{visible.map(r=><tr key={r.yr}><td style={mTD(th)}>{t.yrPrefix||"Yr"} {r.yr}</td><td style={{...mTDR(th),color:th.blue}}>{fmt(r.val)}</td><td style={{...mTDR(th),color:th.neg}}>{fmt(r.bal)}</td><td style={{...mTDR(th),color:th.pos,fontWeight:700}}>{fmt(r.eq)}</td></tr>)}</tbody></table></div><Paginator total={total} page={page} setPage={setPage} perPage={per}/></>;}
...
// ISS-28/29 fix: derive months & interest saved from the SAME monthly amortization as the table.
// (Was: payoff rounded up to whole years (yr*12) vs a closed-form exact month count → moSaved understated / 0 / negative;
//  intSaved a fabricated loanAmt·apr·monthsSaved/12 unrelated to the amort schedule.) Now both schedules use one loop; saved = difference.
const simAmort=(loan,r,pay,extra)=>{let bal=loan,totInt=0,mo=0;const cap=12000;while(bal>0.01&&mo<cap){mo++;const int=bal*r;const prin=Math.min(bal,pay-int+extra);if(prin<=0)return{months:Infinity,totInt:Infinity};totInt+=int;bal=Math.max(0,bal-prin);}return{months:mo,totInt};};
const amBase=am.loanAmt>0&&am.apr>0?simAmort(am.loanAmt,am.apr/100/12,amMo,0):{months:0,totInt:0};
const amExtra=am.loanAmt>0&&am.apr>0?simAmort(am.loanAmt,am.apr/100/12,amMo,am.extra):{months:0,totInt:0};
const moSaved=Number.isFinite(amBase.months-amExtra.months)?Math.max(0,amBase.months-amExtra.months):0;
const intSaved=Number.isFinite(amBase.totInt-amExtra.totInt)?Math.max(0,amBase.totInt-amExtra.totInt):0;
// ── Equity projection
