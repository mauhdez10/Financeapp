...
// ClientDetail header).
function SC({label,value,color,sub}){const th=useTh();const disp=useAnimatedDisplay(value);return<div className="ga-sc" style={{...mCARD(th),padding:"10px 12px",flex:1,minWidth:0,overflow:"hidden"}}><div style={{fontSize:10,color:th.muted,marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"0.06em",textTransform:"uppercase",fontWeight:700}}>{stripLeadEmoji(label)}</div><div style={{fontSize:17,fontWeight:700,color:color||th.accent,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontFamily:"'JetBrains Mono',ui-monospace,Menlo,monospace",fontVariantNumeric:"tabular-nums",lineHeight:1.1}}>{disp}</div>{sub&&<div style={{fontSize:9.5,color:th.dim,marginTop:2,letterSpacing:"0.02em"}}>{sub}</div>}</div>;}
