...
function SC({label,value,color,sub}){const th=useTh();return<div className="ga-sc" style={{...mCARD(th),padding:14,flex:1,minWidth:0,overflow:"hidden"}}><div style={{fontSize:11,color:th.muted,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{label}</div><div style={{fontSize:18,fontWeight:800,color:color||th.accent,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{value}</div>{sub&&<div style={{fontSize:10,color:th.dim,marginTop:2}}>{sub}</div>}</div>;}
...
/* ── MODAL / SAVEBAR / IADD ──────────────────────────────────────────────── */
function Modal({title,onClose,children,width=480,disableBackdropClose=false}){const th=useTh();const{isMobile}=useViewport();return<div onClick={e=>{if(disableBackdropClose)return;if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,background:"#000b",zIndex:1000,display:"flex",alignItems:isMobile?"flex-end":"center",justifyContent:"center",padding:isMobile?0:20}}><div style={{background:th.modal,border:`1px solid ${th.cardBorder}`,borderRadius:isMobile?"16px 16px 0 0":16,padding:isMobile?"18px 16px calc(18px + env(safe-area-inset-bottom,0px))":24,width:"100%",maxWidth:isMobile?"100%":width,maxHeight:isMobile?"92dvh":"90vh",overflowY:"auto",boxShadow:isMobile?"0 -16px 60px #000d":"0 32px 80px #000d"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:isMobile?14:20}}><span style={{fontSize:14,fontWeight:700,color:th.text}}>{title}</span><button onClick={onClose} style={{background:"none",border:"none",color:th.muted,cursor:"pointer",fontSize:24,lineHeight:1,minWidth:36,minHeight:36,touchAction:"manipulation"}}>×</button></div>{children}</div></div>;}
...
function PlanReportBlock({client,lang,t}){
  ...
  return<div style={{...mCARD(th),padding:16,marginBottom:14}}>
    ...
    <div style={{...mCARD(th),padding:14,marginBottom:14,background:net>0?th.pos+"08":th.neg+"08",border:`1px solid ${net>0?th.pos:th.neg}33`}}><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>{[["💼 "+(t.netIncome||"Net Income"),fmt(net),th.pos],["💳 "+(t.totalBills||"Bills"),fmt(bills),th.neg],["🏦 "+(t.minDebtPayAll||"Min Debt Pay (All Loans)"),fmt(totalMin),th.warn],["💰 "+(t.extraPerMo||"Extra/mo"),fmt(extra),extra>0?GOLD:th.neg]].map(([l,v,c])=><div key={l}><div style={{fontSize:10,color:th.dim}}>{l}</div><div style={{fontSize:15,fontWeight:800,color:c}}>{v}</div></div>)}</div></div>
    {hasStrat&&<div style={{fontSize:12,color:th.muted,marginBottom:14,padding:"10px 12px",background:th.accent+"08",borderRadius:8}}>{t.debtStrategyColon||"Debt Strategy:"} <b style={{color:th.accent}}>{strat==="avalanche"?(t.avalancheStrategyDesc||"Avalanche (highest APR first — saves most interest)"):(t.snowballStrategyDesc||"Snowball (smallest balance first — quick wins)")}</b></div>}
    ...
    {investPerMo>0&&<div style={{...mCARD(th),padding:14,marginTop:12,marginBottom:14}}><div style={{fontSize:11,fontWeight:700,color:th.dim,marginBottom:10}}>📈 {t.investmentProjectionHdr||"INVESTMENT PROJECTION"} · starts {addDate(investStart)} · {(investPct*100).toFixed(0)}% of extra cash</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>{[[5,"5 Years"],[10,"10 Years"],[20,"20 Years"]].map(([yrs,label])=><div key={yrs} style={{textAlign:"center"}}><div style={{fontSize:10,color:th.dim,marginBottom:4}}>{label}</div><div style={{fontSize:18,fontWeight:800,color:th.pos}}>{fmt(investFV(yrs))}</div><div style={{fontSize:10,color:th.dim}}>+{fmt(investFV(yrs)-investPerMo*yrs*12)} growth</div></div>)}</div></div>}
...
function FinancialPlanTab({client,onUpdate,t}){
  ...
  return<div><div style={{...mCARD(th),padding:16,marginBottom:16,background:net>0?th.pos+"08":th.neg+"08",border:`1px solid ${net>0?th.pos:th.neg}33`}}><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>{[["💼 "+(t.netIncome||"Net Income"),fmt(net),th.pos],["💳 "+(t.totalBills||"Bills"),fmt(bills),th.neg],["🏦 "+(t.minDebtPayAll||"Min Debt Pay (All Loans)"),fmt(totalMin),th.warn],["💰 "+(t.extraPerMo||"Extra/mo"),fmt(extra),extra>0?GOLD:th.neg]].map(([l,v,c])=><div key={l}><div style={{fontSize:10,color:th.dim}}>{l}</div><div style={{fontSize:15,fontWeight:800,color:c}}>{v}</div></div>)}</div></div>
...
function RemindersPanel({clients,settings,t,onSettingsChange}){
  ...
  return<>{settingsOpen&&<AlertsSettingsModal settings={settings} onSave={onSettingsChange} onClose={()=>setSettingsOpen(false)} t={t}/>}<div style={{...mCARD(th),padding:14,marginTop:16}}><div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>{[["advisor",(t.advisorAlertsLbl||"🔔 Advisor Alerts"),adv.length],["clients",(t.clientDueLbl||"👥 Client Due"),cRem.length]].map(([id,label,count])=><button key={id} onClick={()=>{setTab(id);setPage(1);}} style={{fontSize:11,padding:"4px 12px",borderRadius:8,cursor:"pointer",background:tab===id?th.accent+"22":"transparent",color:tab===id?th.accent:th.muted,border:`1px solid ${tab===id?th.accent:th.cardBorder}`,fontWeight:tab===id?700:400}}>{label}{count>0&&<span style={{background:th.warn,color:"#fff",borderRadius:99,padding:"0 5px",fontSize:10,marginLeft:4}}>{count}</span>}</button>)}<div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:6}}><button onClick={()=>setSettingsOpen(true)} style={{...SEL,fontSize:13}} title={t.alertSettingsTitle||"Alert Settings"}>⚙️</button></div></div>
  {expanded&&<div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}><input placeholder={"🔍 "+(t?.filterByClientPh||"Filter by client...")} value={filtClient} onChange={e=>{setFiltClient(e.target.value);setPage(1);}} style={{...mINP(th),fontSize:11,padding:"4px 10px",width:180}}/>{tab==="advisor"&&<select value={filtType} onChange={e=>{setFiltType(e.target.value);setPage(1);}} style={SEL}><option value="">{t.allTypesOpt||"All Types"}</option><option value="high">{t.highPriorityOpt||"High Priority"}</option><option value="contact">{t.noContactOpt||"No Contact"}</option><option value="debt">{t.debt||"Debt"}</option><option value="cash">{t.cashFlow||"Cash Flow"}</option><option value="promo">{t?.promoFilter||"Promo"}</option></select>}<span style={{fontSize:10,color:th.dim,alignSelf:"center"}}>Sort:</span>{tab==="advisor"?<select value={sortA} onChange={e=>setSortA(e.target.value)} style={SEL}><option value="priority">{t?.sortPriority||"Priority"}</option><option value="client">{t.client||"Client"}</option><option value="task">{t.type||"Type"}</option></select>:<select value={sortC} onChange={e=>setSortC(e.target.value)} style={SEL}><option value="default">{t.dueDateOpt||"Due Date"}</option><option value="client">{t.client||"Client"}</option><option value="task">{t.type||"Type"}</option><option value="amount">{t?.amount||"Amount"}</option></select>}</div>}
...
function ImportWizard({onClose,onImport,existingClients,t}){
  ...
  if(step==='csv_pick'){const filteredCSV=csvClients.filter(c=>`${c.firstName} ${c.lastName} ${c.email}`.toLowerCase().includes(csvSearch.toLowerCase()));return<Modal title={"👥 "+(t?.selectClientTitle||"Select Client Profile")} onClose={onClose} width={520}>
    ...
    <div style={{display:'flex',gap:8,marginBottom:10,alignItems:'center'}}><input placeholder={t?.searchClientsPh||"Search clients…"} value={csvSearch} onChange={e=>setCsvSearch(e.target.value)} style={{...mINP(th),flex:1,padding:'5px 10px',fontSize:12}}/>{mode!=='both'&&<><Btn small onClick={()=>setSelCSV(new Set(csvClients.map(x=>x.id)))}>All</Btn><Btn small onClick={()=>setSelCSV(new Set())}>None</Btn></>}</div>
...
function buildGAPath(nav,selectedId,selectedTab){
  if(selectedId!=null){
    const id=encodeURIComponent(String(selectedId));
    const tab=selectedTab&&selectedTab!=="report"&&_GA_CLIENT_TABS.includes(selectedTab)?("/"+selectedTab):"";
...
function parseGAPath(pathname){
  // returns {nav, clientId, selectedTab} or null when path is /intake (handled separately).
  const p=(pathname||"/").replace(/\/+$/,"")||"/";
  if(p==="/"||p==="")return{nav:"dashboard",clientId:null,selectedTab:"report"};
  ...
  if(parts[0]==="clients"&&parts[1]){
    ...
    return{nav:"clients",clientId,selectedTab:tab};
  }
  if(_GA_NAVS.includes(parts[0]))return{nav:parts[0],clientId:null,selectedTab:"report"};
  return{nav:"dashboard",clientId:null,selectedTab:"report"};
}
...
export default function App(){
  ...
  },[authUser,clients,isPublicIntakeRoute,nav,selectedTab,selected?.id]);
  useEffect(()=>{
    if(typeof window==="undefined")return;
    ...
    const url=buildGAPath(nav,selected?.id??null,selectedTab);
  ...
  useEffect(()=>{
    ...
    if(isPublicIntakeRoute)return;
    const onPop=(e)=>{
      if(drawerOpen){setDrawerOpen(false);window.history.pushState({ga:true,nav,selectedId:selected?.id??null,selectedTab},"",buildGAPath(nav,selected?.id??null,selectedTab));return;}
      const st=e.state;
