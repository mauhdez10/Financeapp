...
function PortalShareModal({client,t,onClose}){
  const th=useTh();
  ...
  const gen=async()=>{if(!uid){setErr("Not signed in. Please reload.");return;}setBusy(true);setErr("");for(const l of links){await gaRevokePortalLink(l.id);}const r=await gaCreatePortalLink(uid,localId,{});if(!r.ok){setErr(r.error||"Failed to create link.");setBusy(false);return;}await reload(uid);setBusy(false);};
  const revoke=async(id)=>{await gaRevokePortalLink(id);if(uid)await reload(uid);};
  ...
  return <Modal title={t.sharePortal||"Share portal"} onClose={onClose} width={520}>
    <div style={{fontSize:12.5,color:th.muted,lineHeight:1.6,marginBottom:18}}>{t.portalModalDesc||"Generate a private, read-only link to this client's financial overview. They can open it without a password. You can revoke it anytime."}</div>
    {active?<div style={{...mCARD(th),padding:16,marginBottom:14}}>
      <div style={{fontSize:10,fontWeight:700,color:th.dim,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{t.portalActiveHdr||"Active link"}</div>
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <button onClick={()=>copy(active.token)} style={{padding:"9px 15px",borderRadius:8,background:th.accent,color:"#1A1208",border:"none",fontWeight:700,fontSize:12,cursor:"pointer"}}>{copied?(t.portalCopiedMsg||"Copied"):(t.portalCopyBtn||"Copy")}</button>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:12}}>
        <span style={{fontSize:10.5,color:th.dim}}>{(active.view_count||0)} {t.portalViewsLbl||"views"}</span>
        <button onClick={()=>revoke(active.id)} style={{fontSize:11,padding:"5px 12px",borderRadius:7,background:"transparent",color:th.neg,border:"1px solid "+th.neg+"44",cursor:"pointer",fontWeight:600}}>{t.portalRevokeBtn||"Revoke"}</button>
      </div>
    ...
    <button onClick={gen} disabled={busy} style={{width:"100%",padding:"12px",borderRadius:10,background:active?"transparent":th.accent,color:active?th.accent:"#1A1208",border:active?"1px solid "+th.accent+"66":"none",fontWeight:700,fontSize:13,cursor:busy?"wait":"pointer"}}>{busy?"…":(active?(t.portalRegenBtn||"Generate new link"):(t.portalGenerateBtn||"Generate portal link"))}</button>
  </Modal>;
}

/* ── PUBLIC CLIENT PORTAL (read-only, token-based) ──────────────────────── */
function PublicPortal(){
  const[mode,setMode]=useState(()=>{try{return localStorage.getItem("ga_portal_mode")||"light";}catch{return "light";}});
  const[lang,setLang]=useState("en");
  const[st,setSt]=useState({loading:true,error:"",client:null,advisor:null});
  const isDark=mode==="dark";
  useEffect(()=>{(async()=>{
    ...
    setSt({loading:false,error:"",client:r.client||{},advisor:r.advisor||{}});
    if(r.lang==="es"||r.lang==="en")setLang(r.lang);
  })();},[]);
