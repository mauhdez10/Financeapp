  ...
  return<>{settingsOpen&&<AlertsSettingsModal settings={settings} onSave={onSettingsChange} onClose={()=>setSettingsOpen(false)} t={t}/>}<div data-ga-grid="two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:16}}>
    ...
    <div style={{...mCARD(th),padding:14}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        <input placeholder={"🔍 "+(t?.searchPh||"Search...")} value={filtClient} onChange={e=>setFiltClient(e.target.value)} style={{...mINP(th),fontSize:11,padding:"4px 10px",flex:"1 1 120px",minWidth:0}}/>
      ...
      {sAdv.length===0?<div style={{fontSize:11,color:th.dim,fontStyle:"italic",padding:"8px 0"}}>{t?.noAdvisorAlerts||"No advisor alerts."}</div>:
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {advVisible.map((a,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:pC(a.priority)+"11",border:`1px solid ${pC(a.priority)}22`}}>
            ...
            <Pill color={pC(a.priority)}>{a.task}</Pill>
          </div>)}
        </div>}
    ...
    <div style={{...mCARD(th),padding:14}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        <input placeholder={"🔍 "+(t?.searchPh||"Search...")} value={filtDue} onChange={e=>setFiltDue(e.target.value)} style={{...mINP(th),fontSize:11,padding:"4px 10px",flex:"1 1 120px",minWidth:0}}/>
...
if(typeof window!=="undefined"){window.__GA_BUILD__="2026-05-22-v0230-header-dedup-clientdue-search-tos-gate-portal-welcome";console.log("%c⚓ Golden Anchor build:","color:#D4A017;font-weight:bold",window.__GA_BUILD__);}
