...
*Last updated: 2026-05-18 — v0.10.1 ad-hoc bug-fix batch shipped (not a queued chat). (1) Supabase client de-duplication — `gaSaveClient`/`gaDeleteClient`/`gaLoadClients` now handle legacy `clients` rows with NULL `local_id` (the cause of the duplicated “Miguel Torres” and the non-persisting delete); paired one-time SQL cleanup `sql/v0.10.1_clients_local_id_repair.sql` must be run in Supabase. (2) Calculators/Resources/About/Services grid sizing restored (v0.9.0 regression). (3) “Clear all” for the Sent invites panel. +2 translation keys × 2 langs; 1,217 → 1,219/side. New pitfall #15. Build marker `2026-05-18-v0101-client-dedup-grids-invites`. Chat 9 (browser back-button / history integration) added to §3 as `queued`. Chat 8 version bump renumbered to v0.10.2 (v0.10.1 now taken). Queue: Chat 8, Chat 9.*
