...
|---|---|---|---|
| 4 | v0.8.0 | 2026-05-16 | Multi-select + bulk actions on the Clients tab. `ClientList` rewritten (now multi-line): every active + archived row gets a leftmost selection checkbox; the row body still opens ClientDetail, only the checkbox toggles selection. New selection bar with "select all visible" + live count + Clear. Selection is component-local `useState(()=>new Set())` of client ids. The ClientList Kebab gains 5 bulk items — 📦 Archive / ↩ Restore / 🗑️ Delete / ✂️ Split… / 🔗 Join…. Archive/Restore/Delete disabled at zero selection (Restore also needs every selected client archived); Split/Join always enabled — they open their own searchable pickers and ignore the selection (single-client operations). Archive/Restore/Delete open confirmation modals listing affected clients by name; bulk Delete requires typing `DELETE`. Split picker → existing `SplitAssignModal`; Join picker → existing `JoinModal`, both reused unchanged. 4 new App handlers: `archiveMany`/`restoreMany`/`deleteMany`/`splitClientPair` (all `useCallback`); `joinClients` reused. `Kebab` gained backward-compatible optional `disabled`-item support; ClientDetail per-client kebab untouched. +27 translation keys × 2 langs; dictionary 1,158 → 1,185 per side. App.jsx 2,743 → 2,832 lines. No SQL migration. |
...
*Last updated: 2026-05-16 — Chat 4 shipped v0.8.0 (multi-select + bulk actions on the Clients tab: per-row selection checkboxes, "select all visible" bar, burger menu with Archive/Restore/Delete/Split/Join, confirmation modals, type-DELETE guard on bulk delete). Chat 4 flipped to `done` in §3. Chats 5 (mobile/responsive redesign) and 6 (Playwright resync) remain `queued`. Chat 5's dependency — Chat 3 (IA changes) `done` — is satisfied, so Chat 5 is now claimable; Chat 6 still waits on Chat 5. Queue now: Chats 5, 6. Backlog unchanged: server-side intake delivery (DNS-blocked) + WhatsApp (long-term) + Future.*
