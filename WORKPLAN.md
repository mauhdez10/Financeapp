...
|---|---|---|---|
| — | v0.12.4 | 2026-05-20 | Patch (out-of-band, not a queued chat). **Email Complete Report PDF section parity + soft-grey background for all three on-screen PrintBtns.** Two coupled fixes for the v0.12.x PDF/print surface, requested by Mauricio after the smoke-test comparison of `golden-anchor-report-Amanda-Chen-2026-05-19.pdf` (email) vs `Golden_Anchor_Finance.pdf` (in-browser print/save). **Problem 1 — email PDF data was wrong AND missing sections.** `buildPrintHTML()` in `api/render-report-pdf.js` from v0.12.0 was reading wrong JSON field names: `.amount` instead of `.netMo` for income (showed $0 monthly on a client whose live dashboard showed $9,600), `.interestRate` instead of `.apr` for debts (APR column showed `—`), only `.name` for bills (missed rows whose label was on `.label`), and aggregated accounts naively (missed the cash $15,800 in the net-worth roll-up; showed $22,000 vs the live $28,100). It was also missing five sections present on-screen: Investment Allocation, Financial Ratios, Cash Flow Statement, Strategy Plan (debt payoff order + 2-phase roadmap + 5/10/20-year investment projection). **Problem 2 — every PDF (email AND in-browser `window.print()` from Monthly + Financial + Complete tabs) was pure white, hard to read.** All three on-screen `PrintBtn` flows go through the global `@media print` CSS, which forced `body { background: white }`. Saved PDFs had no contrast between page and cards. **Fix:** (1) `api/render-report-pdf.js` rewritten (599 → 817 lines): `computeAggregates()` reads `.netMo`-then-`.amount` for income, `.apr` for debts, both `.name` and `.label` for bills; cash accounts (checking/savings) separated for emergency-fund ratio. All five missing sections built out — Investment Allocation (donut + table from `client.investAllocation`), Financial Ratios (3 KPI cards + 3-row benchmark table: Liquidity Ratio, Debt-to-Asset, Emergency Fund), Cash Flow Statement (inflows/outflows grid + operating-cash-flow highlight), Strategy Plan (debt payoff order avalanche table + 2-phase roadmap red/green cards + 3-column investment projection at 8.5%/yr × 5/10/20 years). PDF respects `client.reportInclude` toggle map. Page background `#F1F5F9` (matches `DEF_SETTINGS.lightBg`), sections on white cards with `#E2E8F0` borders. (2) **App.jsx `@media print` block at line 2963 patched (one CSS rule changed):** `body { background: white }` → `html, body { background: #F1F5F9 }`, added `print-color-adjust: exact` (and the WebKit + legacy variants) on `*` so Chrome/Edge/Safari preserve the painted backgrounds when generating the saved PDF — without this rule, browsers strip background colors by default as an ink-saving measure. Applies to all three on-screen PrintBtns automatically (MonthlyReportTab, FinancialStatementReportTab, CompleteReportTab) since they share the global `@media print`. **No App.jsx logic changes, no new components, no translation key changes.** v0.12.3 scope-fix work (D-36) preserved untouched. v0.12.2 Spanish keys (1,313/side) preserved untouched. **Background color choice (option b from design conversation 2026-05-19):** `#F1F5F9` — matches in-app light-mode bg, so the saved PDF and on-screen view feel like the same document. Rejected: `#FAF9F7` (too subtle), `#E2E8F0` (too contrast-heavy). App.jsx 3,046 → 3,046 lines (one CSS rule edit + build marker). `translations.js` unchanged at 1,313 keys/side. `package.json`, `vercel.json` unchanged — reuses v0.12.1's chromium-min runtime. Build marker `2026-05-20-v0124-section-parity-grey-print`. **User-facing note for in-browser print:** Chrome's "Background graphics" checkbox in the print dialog must be checked for the grey to appear in the saved PDF — it's the user-facing toggle that maps to `print-color-adjust: exact`; we set the CSS, browsers still ask the user to opt in. No SQL migration. No new locked decisions, no new pitfalls. D-1, D-7, D-18, D-27, D-30, D-31, D-34, D-36 preserved. |
...
*Last updated: 2026-05-20 — v0.12.4 (Patch, out-of-band, not a queued chat) shipped. **Email Complete Report PDF section parity + soft-grey background for all three on-screen PrintBtns.** Two coupled fixes: (1) `api/render-report-pdf.js` rewritten (599 → 817 lines) to read correct JSON field names (.netMo not .amount, .apr not .interestRate, .label fallback for bills, cash-account aggregation) AND to render the 5 sections the v0.12.0 PDF was missing (Investment Allocation, Financial Ratios, Cash Flow Statement, Strategy Plan with debt payoff + roadmap + projection); respects `client.reportInclude` toggles; page bg `#F1F5F9` with white section cards. (2) App.jsx `@media print` block patched: `body { background: white }` → `html, body { background: #F1F5F9 }` + `print-color-adjust: exact` on `*` so browsers preserve the grey when saving. Applies to all three on-screen PrintBtns. No App.jsx logic changes, no new components, no translation key changes. v0.12.3 scope-fix preserved. translations.js unchanged at 1,313 keys/side. App.jsx 3,046 → 3,046 lines (one CSS rule edit + build marker). Build marker `2026-05-20-v0124-section-parity-grey-print`. **User must enable "Background graphics" in Chrome's print dialog to see the grey on save** (per `print-color-adjust: exact` spec — browsers expose this as a user opt-in checkbox). **Queue:** Chat 11 (PDF polish + O-14 prep) — still `queued`, claimable now; v0.12.4 closed the "PDF data wrong + report parity" item that was part of Chat 11's spec, so Chat 11 is now scoped to engagement-letter gate (O-14 part A/B) + any further PDF visual polish. §4 backlog unchanged.*

*Prior: 2026-05-20 — v0.12.3 (Hotfix for v0.12.2 t-out-of-scope crash) shipped. v0.12.2 broke production immediately after deploy — blank screen after login, every user. 8 component functions (Kebab/PTag/BulkSnapModal/ImportWizard/DuplicateResolverModal/DeleteClientModal/BackupImportModal/ExportModal) referenced t.xxx without t in their params → ReferenceError → React crash. Mauricio rolled back to v0.12.1. v0.12.3 added t to all 8 signatures + 22 t={t} call sites, with a rewritten brace-aware JSX patcher and a new scope-aware static verifier (both checks now pass zero). Belt-and-suspenders: 177 sites using v0.12.2-introduced keys wrapped with optional chaining (t?.X||"fallback"), so a future scope regression degrades to the fallback string instead of ReferenceError — pre-existing t.X|| sites left untouched (battle-tested). New locked decision D-36 (scope-aware checker mandatory for bulk static-text patches). translations.js unchanged. App.jsx 3,046→3,046 lines. Build marker `2026-05-20-v0123-t-scope-fix`. **Queue:** Chat 11 (PDF polish + O-14 prep) — still `queued`, claimable now. §4 backlog unchanged.*
