...
- **Send intake form to prospect (Chat 7, planned).** Add a "Send Intake Link" surface to the `IntakeSubmissionsPage` header. Inputs: prospect email and/or phone, optional name, optional pre-filled language preference. Outputs: an `intake_invites` row (or `intake_submissions` row with status `invited`) carrying the prospect contact + a unique token, plus a delivery via (a) email (Resend or similar) and/or (b) WhatsApp (Twilio WhatsApp API) and/or (c) SMS (Twilio). The form URL becomes `https://finance.goldenanchor.life/intake?advisor=<uuid>&invite=<token>` so the resulting submission is pre-tagged to the invite. Open decisions before this can start: (1) which email provider — Resend, SendGrid, AWS SES, or Postmark — and how billing fits Mauricio's pre-revenue stage; blocked on Porkbun→Cloudflare DNS migration since Resend needs DNS records for the `goldenanchor.life` sender domain; (2) WhatsApp Business API approval is non-trivial — Twilio requires a verified Business profile, may take days/weeks; SMS via Twilio is cheaper and immediately available but loses the "share-a-tappable-link" UX; (3) simplest MVP path: a "Copy link with prospect's name pre-filled" button + a one-click `mailto:` opener with a pre-filled subject/body — no server needed, no DNS migration, ships today; advisor sends from their own Gmail. Recommend MVP first, then upgrade to server-side delivery in Chat 8 after DNS is sorted.
...
*Last updated: 2026-05-16 — Chat 3 shipped v0.7.0 → v0.7.1 → v0.7.2 across three sub-sessions: v0.7.0 IA refactor (Forms tab deleted, per-client Intake tab merged into MonthlyTab), v0.7.1 full-parity public intake form + Edit/Delete submissions, v0.7.2 SSN auto-format + theme fix on reused editors + sort-arrow spacing. Chat 6 (Playwright resync) still in §3 queue. New backlog item: "Send intake form to prospect" feature (email/WhatsApp/SMS delivery + invite tokens) — sized as Chat 7, blocked on DNS migration for email path. Queue now: Chats 4, 5, 6. Backlog: send-intake-to-prospect + Future.*
