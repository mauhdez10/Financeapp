...
**Version bump:** patch → v0.10.2 (if translation fixes ship) or none (if Spanish audit finds nothing wrong and only docs change). (v0.10.1 was used by the 2026-05-18 ad-hoc bug-fix batch.)
...
|---|---|---|---|
| 9 | v0.11.0 | 2026-05-19 | Browser history / back-button integration. The app is a single-page app; tab/section navigation (`nav`, `selected`, `selectedTab`) was plain React state with no `history` integration, so the browser kept no in-app entries and Back left the app entirely (landed before login). **Fix:** a navigation-signature `useEffect` keyed on `[nav, selected?.id, selectedTab, authUser, isPublicIntakeRoute]` pushes a history entry (`{ga:true,nav,selectedId,selectedTab}`) on each in-app navigation change — first run `replaceState` to seed the entry point, later runs `pushState` — instead of instrumenting ~10 individual `setNav`/`setSelected` call sites. A `popstate` listener restores nav/selectedTab/selected (client looked up by id from `clients`); a no-state `popstate` falls back to the dashboard rather than unloading the SPA. A `_popstateRestoringRef` guard suppresses the push on the render caused by a restore, so Back/Forward don't spawn duplicate entries. An open mobile drawer is closed by the first Back. Both effects sit in the effects block below every hook declaration and above the `isPublicIntakeRoute` early return (pitfall #13 honored); the push effect early-returns on the `/intake` route and while signed out, so the `!authUser` → `<Login/>` gate is unaffected. Out of scope: deep-linkable `/clients/<id>` URLs. translations.js unchanged. App.jsx 2,965 → 3,001 lines. New pitfall #16. No new/closed locked decisions. Build marker `2026-05-19-v0110-history-backbutton`. |
| — | v0.10.2 | 2026-05-18 | Ad-hoc one-line code fix (not a queued chat), on top of v0.10.1. **`gaDeleteClient` rewritten.** v0.10.1's soft-delete used a single `.or("local_id.eq.${id},data->>id.eq.${id}")` filter; PostgREST parses an `or` filter by splitting the string on `.`, so the JSON path `data->>id` and any decimal client id (e.g. `1776873994030.0803`, a real legacy id) corrupt the query → 400 → the soft-delete silently no-ops → client reappears on refresh. Archive was unaffected (it routes through `gaSaveClient`, which uses `.eq()`). Fix: two plain `.eq()` UPDATE calls (`local_id`, then `data->>id` fallback). **Correction to the v0.10.1 record:** its stated root cause ("legacy rows with NULL `local_id`") was wrong — live data shows `local_id` populated on every row; `sql/v0.10.1_clients_local_id_repair.sql` was a no-op and was correctly never committed. The "client duplication" launch-blocker was investigated against live data and found to be cross-account confusion (`local_id` `1` exists under both Mauricio's account and the Playwright test account, different names) — no real duplicate rows within an account. v0.10.1's grid / invites / de-dupe changes remain valid. translations.js unchanged; App.jsx line count unchanged. Pitfall #15 amended. Build marker `2026-05-18-v0102-delete-fix`. |
...
*Last updated: 2026-05-19 — v0.11.0 (Chat 9) `done`. Browser history / back-button integration: in-app navigation now pushes browser history (navigation-signature `useEffect` → `pushState`, first run `replaceState`; `popstate` listener restores nav/selected/tab; `_popstateRestoringRef` guard prevents re-push loops; open mobile drawer closed by first Back). Effects placed below all hooks, above the `isPublicIntakeRoute` early return (pitfall #13). translations.js unchanged. App.jsx 2,965 → 3,001 lines. New pitfall #16. Build marker `2026-05-19-v0110-history-backbutton`. Prior: v0.10.2 (Patch, ad-hoc) — `gaDeleteClient` `.or()` → two `.eq()` delete fix; corrected the v0.10.1 NULL-`local_id` misdiagnosis; the “client duplication” report was found to be cross-account test data, not a real bug. Queue: Chat 8. Backlog: Email Complete Report + PDF (O-11), Twilio activation, WhatsApp (long-term).*
